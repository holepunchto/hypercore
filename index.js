...
class Hypercore extends EventEmitter {
  ...
  [inspect](depth, opts) {
    ...
    for (let i = 0; i < min; i++) {
      ...
      peers +=
        indent +
        '      remoteLength: ' +
        opts.stylize(peer.remoteLength, 'number') +
        '\n'
      peers +=
        indent +
        '      remoteFork: ' +
        opts.stylize(peer.remoteFork, 'number') +
        '\n'
      peers +=
        indent +
        '      remoteCanUpgrade: ' +
        opts.stylize(peer.remoteCanUpgrade, 'boolean') +
        '\n'
      peers += indent + '    )' + '\n'
    }
  ...
  static key(manifest, { compat, version, namespace } = {}) {
    ...
    return compat
      ? manifest.signers[0].publicKey
      : manifestHash(createManifest(manifest))
  }
  ...
  static createCore(storage, opts) {
    return new Core(Hypercore.defaultStorage(storage), {
      autoClose: false,
      ...opts
    })
  }
  ...
  session(opts = {}) {
    ...
    const writable =
      opts.writable === undefined ? !this._readonly : opts.writable === true
  ...
  async setEncryption(encryption) {
    ...
    if (!isEncryptionProvider(encryption)) {
      throw ASSERTION(
        'Provider does not satisfy HypercoreEncryption interface',
        this.discoveryKey
      )
    }
  ...
  async _openSession(opts) {
    ...
    if (opts.atom) {
      ...
      this.state = await parentState.createSession(
        opts.name,
        !!opts.overwrite,
        null
      )
    ...
    if (this.state && checkout !== -1) {
      if (!opts.name && !opts.atom)
        throw ASSERTION(
          'Checkouts must be named or atomized',
          this.discoveryKey
        )
  ...
  replicate(isInitiator, opts = {}) {
    ...
    if (Protomux.isProtomux(isInitiator))
      return this._attachToMuxer(isInitiator)
  ...
  async update(opts) {
    ...
    const remoteWait = this._shouldWait(
      opts,
      this.core.replicator.findingPeers > 0
    )
    ...
    if (!upgraded && remoteWait) {
      const activeRequests =
        (opts && opts.activeRequests) || this.activeRequests
  ...
  async seek(bytes, opts) {
    ...
    if (!isValidIndex(bytes))
      throw ASSERTION('seek is invalid', this.discoveryKey)
    ...
    const timeout =
      opts && opts.timeout !== undefined ? opts.timeout : this.timeout
  ...
  async get(index, opts) {
    ...
    if (!isValidIndex(index))
      throw ASSERTION('block index is invalid', this.discoveryKey)

    if (this.closing !== null)
      throw SESSION_CLOSED('cannot get on a closed session', this.discoveryKey)

    const encoding =
      (opts && opts.valueEncoding && c.from(opts.valueEncoding)) ||
      this.valueEncoding
  ...
  async clear(start, end = start + 1, opts) {
    ...
    if (this.closing !== null)
      throw SESSION_CLOSED(
        'cannot clear on a closed session',
        this.discoveryKey
      )
  ...
  async truncate(newLength = 0, opts = {}) {
    ...
    const writable =
      !this._readonly && !!(signature || (keyPair && keyPair.secretKey))
    if (
      isDefault &&
      writable === false &&
      (newLength > 0 || fork !== this.state.fork)
    )
      throw SESSION_NOT_WRITABLE(
        'cannot append to a non-writable core',
        this.discoveryKey
      )
  ...
  async append(blocks, opts = {}) {
    ...
    if (this._readonly || writable === false)
      throw SESSION_NOT_WRITABLE(
        'cannot append to a readonly core',
        this.discoveryKey
      )
    ...
    return this.state.append(buffers, {
      keyPair,
      signature,
      preappend,
      maxLength
  ...
  _getEncryptionProvider(e) {
    ...
    return new DefaultEncryption(e.key, this.key, {
      block: e.block,
...
function maybeUnslab(block) {
  ...
  return block !== null && 2 * block.byteLength < block.buffer.byteLength
    ? unslab(block)
    : block
}
...
function getEncryptionOption(opts) {
  ...
  return b4a.isBuffer(opts.encryption)
    ? { key: opts.encryption }
    : opts.encryption
}
...
function isEncryptionProvider(e) {
  return (
    e && isFunction(e.padding) && isFunction(e.encrypt) && isFunction(e.decrypt)
  )
}
