...
export interface HypercoreGetManifestOptions {
  /** Return the manifest in its encoded (buffer) form instead of the parsed object. */
  raw?: any
}
...
export class Hypercore {
  ...
  /**
   * Parse and validate a manifest, returning the parsed form or `null` if it is invalid. Throws if the quorum exceeds the number of signers, or if `key` is provided and the manifest does not hash to it.
   */
  static parseManifest(manifest: any, key?: any): any
  ...
  /**
   * Returns the core's manifest, or `null` if it is not available.
   */
  getManifest(opts?: HypercoreGetManifestOptions): any
