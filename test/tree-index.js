...
const tree = require('../lib/tree-index')
...
tape('set and get', function (t) {
  ...
  index = tree()
  for (let i = 0; i < 32; i += 2) index.set(i)
...
tape('chaos monkey randomized', function (t) {
  let blocks = 0
  const source = tree()
  const peers = [source, tree(), tree(), tree(), tree(), tree(), tree(), tree(), tree()]
  ...
  for (let i = 0; i < 5000; i++) {
    ...
    if (Math.random() < 0.5) {
      const a = peers[(Math.random() * peers.length) | 0]
      const b = peers[(Math.random() * peers.length) | 0]

      if (!a.have || a === b) continue
      const want = a.have[(a.have.length * Math.random()) | 0]
...
function copy (block, from, to, t) {
  const proof = from.proof(2 * block, { digest: to.digest(2 * block) })
  let i = 0
  ...
  if (proof.verifiedBy) {
    const roots = flat.fullRoots(proof.verifiedBy)
