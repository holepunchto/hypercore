...
test('announce-range-on-update flow with big core (multiple bitfield pages)', async function (t) {
  ...
  t.is(getPeer(c, b)._remoteHasBlock(lastBlock), false, 'Sanity check: c knows nothing yet')
  t.is(getPeer(b, a)._remoteHasBlock(lastBlock), true, 'Sanity check: b knows about a')
