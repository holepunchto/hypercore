...
test('basic replication stats', async function (t) {
  ...
  t.is(
    aStats.wireSync.rx,
    bStats.wireSync.tx,
    'wireSync received == transmitted'
  )

  t.ok(aStats.wireRequest.rx > 0, 'wireRequests incremented')
  t.is(
    aStats.wireRequest.rx,
    bStats.wireRequest.tx,
    'wireRequests received == transmitted'
  )
  ...
  t.is(
    aStats.wireRange.tx,
    bStats.wireRange.rx,
    'wireRange received == transmitted'
  )
  ...
  t.is(
    aStats.wireExtension.tx,
    bStats.wireExtension.rx,
    'extension received == transmitted'
  )
  ...
  t.is(
    bStats.wireBitfield.tx,
    cStats.wireBitfield.rx,
    'bitfield received == transmitted'
  )

  t.is(
    initStatsLength,
    [...Object.keys(aStats)].length,
    'No stats were dynamically added'
  )
...
test('cancel block', async function (t) {
  ...
  t.ok(
    a.core.replicator.stats.wireCancel.rx > 0,
    'wireCancel stats incremented'
  )
  t.is(
...
test('uses hotswaps to avoid long download tail', async (t) => {
  ...
  const [fastStream] = replicateDebugStream(core, peerCore, t, {
    speed: 10_000_000
  })
  const [slowStream] = replicateDebugStream(slowCore, peerCore, t, {
    speed: 1_000_000
  })
  ...
  t.ok(
    slowPeer.stats.wireData.rx > 0,
    'sanity check: still received data from slow peer'
  )
})
...
test('messages exchanged when 2 sparse cores connect', async function (t) {
  ...
  t.is(
    sparse2.replicator.peers.length,
    1,
    'only connected to the sparse peer (sanity check)'
  )
...
test('range is broadcast when a core is fully available', async function (t) {
  ...
  t.is(
    ...
    1,
    'transmitted range when connection opened'
  )
  t.is(
    reader.replicator.stats.wireRange.tx,
    0,
    'reader did not transmit range yet'
  )
  ...
  t.is(
    reader.replicator.stats.wireRange.tx,
    1,
    'reader transmitted range when fully downloaded'
  )
  ...
  t.is(
    reader.replicator.stats.wireRange.tx,
    1,
    'no new broadcast range since not fully downloaded'
  )
...
test('range is broadcast when a core is fully available (multiple peers)', async function (t) {
  ...
  t.is(
    reader1ToReader2.remoteContiguousLength,
    3,
    'reader2 broadcast to reader1 too'
  )
  t.is(
    reader2ToReader1.remoteContiguousLength,
    3,
    'reader1 broadcast to reader2 too'
  )
...
async function waitForRequestBlock(core) {
  while (true) {
    const reqBlock = core.core.replicator._inflight._requests.find(
      (req) => req && req.block
    )
    if (reqBlock) break
