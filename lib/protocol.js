...
Channel.prototype.close = function (err) {
  ...
  for (var i = 0; i < this._buffers.length; i++) {
    this.stream._parse(this._buffers[i])
  }
}

Channel.prototype.bitfield = function (bitfield) {
  this._send(2, bitfield)
}
...
Channel.prototype.cork = function () {
  this._corked++
}

Channel.prototype.uncork = function () {
  if (--this._corked) return
  if (!this._buffers) return

  while (this._buffers.length && !this._corked) {
...
Channel.prototype._parse = function (buf, offset) {
  if (this._corked) {
    ...
    if (!this.key && this._buffers.length > 16) {
      this.stream.destroy(new Error('Buffer overflow'))
    }
    return
  }
  ...
  switch (type) {
    ...
    case 2: return this.emit('bitfield', message)
    case 3: return this.emit('have', message)
    case 4: return this.emit('pause')
    case 5: return this.emit('resume')
...
Channel.prototype._onhandshake = function (message) {
  this.stream.remoteId = message.peerId || crypto.randomBytes(32)
  this.stream.emit('handshake', message)
...
Channel.prototype._open = function (feed) {
  ...
  if (!this.stream.remoteId) {
    this.handshake({peerId: this.stream.id})
  }

  this.stream.emit('channel', this)
  this.uncork()
}
