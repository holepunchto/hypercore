...
Protocol.prototype.open = function (key, publicId, feed) {
  ...
  this._sendRaw(channel.local, messages.Open.encode(channel))

  if (this._first) {
    this._first = false
    this.send(channel.local, 0, {peerId: this.id, extensions: this.extensions})
    this._decode.resume()
...
Protocol.prototype._parse = function (data) {
  ...
  if (type === 0) return this._onhandshake(message)
  if (type === 8) return this._close(channel)
