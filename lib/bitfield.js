...
module.exports = class Bitfield {
  ...
  * want (start, length) {
  ...
  async flush () {
    if (!this.unflushed.length) return

    const write = this.storage.createWriteBatch()
    ...
    for (const page of this.unflushed) {
      ...
      page.dirty = false
      write.putBitfieldPage(page.index, buf)
    }
  ...
  static async open (storage) {
    const pages = new BigSparseArray()
    ...
    const str = storage.createBitfieldPageStream()

    for await (const { index, page } of str) {
      buffer.set(page, index * BYTES_PER_PAGE)
    }

    return new Bitfield(storage, buffer)
  }
}
