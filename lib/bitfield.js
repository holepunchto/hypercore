...
class BitfieldPage {
  ...
  set (index, val) {
    if (this.dirty === null) this.dirty = new Map()

    this.dirty.set(index, (val ? 1 : -1) * (index + 1))
  }
  ...
  setRange (start, length, val) {
    ...
    this.dirty.set(start, (val ? 1 : -1) * (start + length))
  }
  ...
  * want (start, length) {
  ...
  flush (writer) {
    ...
    for (const page of this.unflushed) {
      ...
      for (const [start, end] of page.dirty) {
        const val = end > 0
        const j = val ? end : (end * -1)
  ...
  setFlushed (flushed) {
    for (const [index, ranges] of flushed) {
      ...
      for (const [start, end] of ranges) {
        ...
        if (j === start + 1) page._set(start, val)
        else page._setRange(start, j - start, val)
      }
    }
  }
