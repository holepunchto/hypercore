...
class Intervals {
  ...
  get (index) {
    if (this.cleared) {
      ...
      return (r.start < index && index < r.end) ? -1 : 0
    }

    let btm = 0
    let top = this.ranges.length - 1
    ...
    while (true) {
      ...
      if (r.start <= index && index < r.end) return 1
...
class BitfieldPage {
  ...
  get (index, dirty) {
    if (dirty && this.dirty) {
      const val = this.dirty.get(index)
  ...
  set (index, val) {
    if (this.dirty === null) this.dirty = new Intervals()

    if (val) this.dirty.add(index, index + 1)
    else this.dirty.clear(index, index + 1)
  }
  ...
  setRange (start, length, val) {
    ...
    if (val) this.dirty.add(start, start + length)
    else this.dirty.clear(start, start + length)
  }
  ...
  * want (start, length) {
  ...
  flush (writer) {
    ...
    for (const page of this.unflushed) {
      ...
      for (const { start, end } of page.dirty.ranges) {
  ...
  setFlushed (flushed) {
    for (const [index, intervals] of flushed) {
      const page = this._pages.get(index)

      for (const { start, end } of intervals.ranges) {
        const val = intervals.cleared === false
