...
class BitfieldPage {
  ...
  get (index, dirty) {
    if (dirty && this.dirty) {
      for (const { start, end, val } of this.dirty) {
        if (start <= index && end > index) return val
      }
    }
  ...
  set (index, val) {
    if (this.dirty === null) this.dirty = []

    addIntervalSorted(this.dirty, index, index + 1, val)
  }
  ...
  setRange (start, length, val) {
    ...
    addIntervalSorted(this.dirty, start, start + length, val)
  }

  _setRange (start, length, val) {
...
module.exports = class Bitfield {
  ...
  get (index, dirty = false) {
    const j = index & (BITS_PER_PAGE - 1)
    const i = (index - j) / BITS_PER_PAGE

    const p = this._pages.get(i)

    return p ? p.get(j, dirty) : false
  }
  ...
  * want (start, length) {
  ...
  clear (writer) {
    return writer.deleteBitfieldPageRange(0, -1)
  }
  ...
  flush (writer) {
    if (!this.unflushed.length) return null

    const flushed = new Map()

    for (const page of this.unflushed) {
      const buf = b4a.allocUnsafe(page.bitfield.buffer.byteLength)
      buf.set(page.bitfield.buffer, page.bitfield.byteOffset)

      for (const { start, end, val } of page.dirty) {
      ...
      flushed.set(page.index, page.dirty)

      page.dirty = null

      // todo: we should memcpy here in case buf is updated afterwards
  ...
  setFlushed (flushed) {
    for (const [index, ranges] of flushed) {
      const page = this._pages.get(index)

      for (const { start, end, val } of ranges) {
        if (end === start + 1) page._set(start, val)
        else page._setRange(start, end - start, val)
      }
    }
  }
  ...
  static async open (storage) {
    ...
    const stream = storage.createBitfieldPageStream()
...
function addIntervalSorted (arr, start, end, val) {
  const interval = { start, end, val }

  for (let i = 0; i < arr.length; i++) {
    if (start <= arr[i].start) {
      if (end < arr[i].start) {
        arr.splice(i, -1, interval)
        return
      }

      if (arr[i].val === val) {
        arr[i].start = start
        if (end > arr[i].end) arr[i].end = end
        return
      }
    ...
    if (end <= arr[i].end) {
      if (arr[i].val === val) return

      const prev = arr[i].end
      arr[i].end = start

      arr.splice(i + 1, -1, interval)

      if (prev !== end) {
        arr.splice(i + 2, -1, { start: end, end: prev, val: arr[i].val })
      }
