...
module.exports = class MerkleTree {
  constructor (storage, roots, fork, signature, staticTree) {
    this.crypto = crypto
    this.fork = fork
    this.roots = roots
    ...
    this.staticTree = staticTree || null
  ...
  static async open (storage, opts = {}) {
    ...
    return new MerkleTree(storage, roots, opts.fork || 0, opts.signature || null, opts.staticTree)
  }
}
...
function verifyUpgrade ({ fork, upgrade }, blockRoot, batch) {
  const staticTree = batch.tree.staticTree

  if (staticTree) {
    const { start, length } = upgrade
    if (start < staticTree.length && (start !== 0 || length !== staticTree.length)) {
      throw INVALID_PROOF('Upgrade does not satisfy static tree')
    }
  }
  ...
  if (staticTree && batch.length === staticTree.length) {
    if (!b4a.equals(staticTree.hash, batch.hash())) {
...
async function generateProof (tree, block, hash, seek, upgrade) {
  ...
  if (tree.staticTree && upgrade) {
    upgrade.start = upgrade.start < tree.staticTree.length ? 0 : upgrade.start
    upgrade.length = upgrade.start < tree.staticTree.length ? tree.staticTree.length : upgrade.length
  }
