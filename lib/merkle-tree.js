...
class MerkleBatch {
  ...
  async verify ({ seek, block, upgrade }) {
    ...
    if (!this.truncated && upgrade.start !== this.tree.length) {
    ...
    this.upgraded = this.truncated || (this.length > this.expectedLength)
  ...
  commit () {
    ...
    if (this.truncated) this.tree._truncate(this.truncateTo)
...
module.exports = class MerkleTree {
  ...
  get (index, error = true) {
    const c = (this.truncationNeeded && index >= this.truncateTo && !this.unflushed.has(index))
      ? { index, size: 0, hash: BLANK_HASH }
      : this.unflushed.get(index)
  ...
  flush () {
    ...
    return new Promise((resolve, reject) => {
      ...
      let truncateTo = this.truncateTo

      const flushed = []
      const self = this
      const slab = Buffer.allocUnsafe(this.unflushed.size * 40)

      if (!this.truncationNeeded) return write(null)
      this.storage.del(treeSize(truncateTo), Infinity, write)

      function write (err) {
        if (err) return reject(err)

        if (self.truncateTo < truncateTo) {
          truncateTo = self.truncateTo
          self.storage.del(treeSize(truncateTo), Infinity, write)
          return
        }

        self.truncateTo = 0
        self.truncationNeeded = false

        for (const node of self.unflushed.values()) {
          flushed.push(node)
          const b = slab.slice(offset, offset += 40)
          uint64le.encode(node.size, b, 0)
          node.hash.copy(b, 8)
          self.storage.write(node.index * 40 + 32, b, done)
        }
  ...
  _truncate (len) {
    ...
    for (const node of this.unflushed.values()) {
      ...
      this.unflushed.delete(node.index)
    }

    if (this.truncationNeeded === true) {
      this.truncateTo = Math.min(this.truncateTo, len)
      return
    }

    this.truncationNeeded = true
    this.truncateTo = len
  }
  ...
  async proof ({ seek, block, upgrade }, signature = null) {
    ...
    if (!seek && !block && !upgrade) {
      upgrade = { start: 0, length: this.length }
    }
...
function treeSize (len) {
  return len === 0 ? 0 : (len - 1) * 80 + 32 + 40
}
