...
class MerkleTreeBatch {
  ...
  commit (writer) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      writer.putTreeNode(this.batchId, node)
    }
  }
  ...
  _commitUpgrade (writer) {
    ...
    if (this.ancestors < this.treeLength) {
      writer.deleteTreeNodeRange(this.batchId, this.ancestors, this.treeLength)
      ...
      while (true) {
        if (ite.contains(head) && ite.index < head) {
          writer.deleteTreeNode(this.batchId, ite.index)
        }
  ...
  byteRange (index, reader = null) {
    return getByteRange(this, index, reader)
  }

  byteOffset (index, reader = null) {
    if (index === 2 * this.tree.length) return this.tree.byteLength
    return getByteOffset(this, index, reader)
  }
}
...
module.exports = class MerkleTree {
  ...
  getRoots (length) {
    ...
    for (let i = 0; i < indexes.length; i++) {
      roots[i] = reader.getTreeNode(this.batchId, indexes[i], true)
    }
  ...
  async upgradeable (length) {
    ...
    for (let i = 0; i < indexes.length; i++) {
      roots[i] = reader.getTreeNode(this.batchId, indexes[i], false)
    }
  ...
  get (index, error = true, reader = null) {
    if (reader) return reader.getTreeNode(this.batchId, index, error)

    return this.storage.getTreeNode(this.batchId, index, error)
  }

  clear (writer) {
    const wb = writer || this.storage.createWriteBatch()
    const promise = wb.deleteTreeNodeRange(this.batchId, 0, -1)
...
async function getNodeSize (tree, index, reader) {
  return (await reader.getTreeNode(tree.batchId, index, true)).size
}

async function getByteOffset (tree, index, reader) {
  ...
  for (const node of tree.roots) { // all async ticks happen once we find the root so safe
    ...
    if (reader === null) reader = tree.storage.createReadBatch()
    const promises = []
    ...
    while (ite.index !== index) {
      if (index < ite.index) {
        ...
        promises.push(tree.get(ite.leftChild(), true, reader))
        ite.sibling()
      }
    }
...
function getByteRange (tree, index, reader) {
  ...
  const offset = tree.byteOffset(index, reader)
  const size = getNodeSize(tree, index, reader)
...
async function generateProof (tree, block, hash, seek, upgrade) {
  ...
  if (node !== null && (!upgrade || node.lastIndex < upgrade.start)) {
    ...
    blockAndSeekProof(tree, reader, node, seek, seekRoot, subTree, p)
  } else if ((node || seek) && upgrade) {
    subTree = seek ? await seekFromHead(tree, to, seek.bytes, seek.padding) : node.index
  }

  if (upgrade) {
    upgradeProof(tree, reader, node, seek, from, to, subTree, p)
    if (head > to) additionalUpgradeProof(tree, reader, to, head, p)
  }
