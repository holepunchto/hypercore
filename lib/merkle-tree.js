...
class MerkleTreeBatch {
  ...
  _commitUpgrade (batch) {
    ...
    if (this.ancestors < this.treeLength) {
      batch.deleteTreeNodeRange(this.ancestors, this.treeLength)
...
module.exports = class MerkleTree {
  ...
  get (index, error = true, batch = null) {
    ...
    if (batch) return batch.getTreeNode(index, error)
  ...
  clear () {
    ...
    batch.deleteTreeNodeRange(0, this.length)
  ...
  static async open (storage, opts = {}) {
    const last = await storage.peakLastTreeNode()

    const length = typeof opts.length === 'number' ? opts.length : last ? last.index : 0
...
function getByteRange (tree, index, batch) {
  ...
  const size = getNodeSize(batch, index)
