  ...
  shift(index) {
    ...
    if (node.index !== index) {
      throw INVALID_OPERATION(
        'Expected node ' + index + ', got node ' + node.index
      )
    }
...
class MerkleTreeBatch {
  ...
  signableCompat(noHeader) {
    return caps.treeSignableCompat(
      this.hash(),
      this.length,
      this.fork,
      noHeader
  ...
  commit(tx) {
    ...
    if (!this.commitable())
      throw INVALID_OPERATION(
        'Tree was modified during batch, refusing to commit'
      )
  ...
  async restore(length) {
    ...
    const roots = unslabNodes(
      await MerkleTree.getRootsFromStorage(this.storage, length)
    )
...
class ReorgBatch extends MerkleTreeBatch {
  ...
  async _update(nodes) {
    ...
    while ((ite.index & 1) !== 0) {
      ...
      const existing = await getTreeNodeFromStorage(
  ...
  _updateDiffRoot(diff) {
    ...
    if (
      (diff.index & 1) === 0 ||
      this.want.start >= this.treeLength ||
      len <= 0
    ) {
      this.want = null
      return true
...
class ByteSeeker {
  ...
  async _seek(bytes) {
    ...
    for (const node of this.session.roots) {
      ...
      while ((ite.index & 1) !== 0) {
        const l = await getTreeNodeFromStorage(
          this.session.storage,
          ite.leftChild()
        )
...
class TreeProof {
  ...
  async settle() {
    ...
    const [pNode, pSeek, pUpgrade, pAdditional] = await settleProof(
      this.pending
    )
...
class MerkleTree {
  ...
  static async truncate(session, length, batch, fork = batch.fork) {
    ...
    for (let i = 0; i < fullRoots.length; i++) {
      ...
      batch.roots.push(
        unslabNode(await getTreeNodeFromStorageOrError(session.storage, root))
      )
    }
  ...
  static async verify(session, proof) {
    ...
    if (unverified) {
      const verified = await getTreeNodeFromStorageOrError(
        session.storage,
        unverified.index
      )
  ...
  static async missingNodes(session, index, length) {
    ...
    while (
      !ite.contains(head) &&
      !(await hasTreeNode(session.storage, ite.index))
    ) {
      cnt++
      if (cnt >= 1024)
        throw ASSERTION(
          'Bad arguments to missingNodes index=' +
            index +
            ' at length=' +
            length
        )
...
function getByteOffsetBatch(roots, index, rx) {
  ...
  for (const node of roots) {
    ...
    while (ite.index !== index) {
      if (++cnt >= 1024)
        throw ASSERTION('Bad getByteOffsetSession index=' + index)
...
function verifyTree({ block, hash, seek }, nodes) {
  ...
  const blockHash =
    untrustedNode.value && blockNode(ite.index, untrustedNode.value)
...
async function seekTrustedTree(session, root, bytes, padding) {
  ...
  while ((ite.index & 1) !== 0) {
    if (++cnt >= 1024)
      throw ASSERTION(
        'Bad seekTrusted bytes=' + bytes + ', paddding=' + padding
      )
...
function seekProof(session, rx, seekRoot, root, p) {
  ...
  while (ite.index !== root) {
    if (++cnt >= 1024)
      throw ASSERTION('Bad seekProof seekRoot=' + seekRoot + ', root=' + root)
    ite.sibling()
    p.seek.push(getTreeNodeOrError(rx, ite.index))
    ite.parent()
  }
...
function blockAndSeekProof(session, rx, node, seek, seekRoot, root, p) {
  ...
  while (ite.index !== root) {
    if (++cnt >= 1024)
      throw ASSERTION(
        'Bad blockAndSeekProof seekRoot=' + seekRoot + ', root=' + root
      )
    ite.sibling()
  ...
  for (const ite = flat.iterator(0); ite.fullRoot(to); ite.nextTree()) {
    ...
    if (p.upgrade === null && ite.contains(from - 2)) {
      ...
      while (ite.index !== root) {
        if (++cnt >= 1024)
          throw ASSERTION(
            'Bad upgradeProof target=' + target + ', root=' + root
          )
  ...
  for (const ite = flat.iterator(0); ite.fullRoot(to); ite.nextTree()) {
    ...
    if (p.additionalUpgrade === null && ite.contains(from - 2)) {
      ...
      while (ite.index !== root) {
        if (++cnt >= 1024)
          throw ASSERTION(
            'Bad arguments to additionalUpgradeProof root=' +
              root +
              ' target=' +
              target
          )
...
async function getTreeNodeOrError(rx, index) {
  ...
  if (node === null)
    throw INVALID_OPERATION(
      'Expected tree node ' + index + ' from storage, got (nil)'
    )
  return node
}
...
async function generateProof(session, rx, block, hash, seek, upgrade) {
  ...
  if (session.prologue && upgrade) {
    ...
    upgrade.length =
      upgrade.start < session.prologue.length
        ? session.prologue.length
        : upgrade.length
  }
  ...
  if (node !== null && (!upgrade || node.lastIndex < upgrade.start)) {
    ...
    subTree = seek
      ? await seekFromHead(session, to, seek.bytes, seek.padding)
      : node.index
  }
...
function getUnpaddedSize(node, padding, ite) {
  return padding === 0
    ? node.size
    : node.size -
        padding * (ite ? ite.countLeaves() : flat.countLeaves(node.index))
}
