...
const RocksStorage = require('hypercore-on-the-rocks')
      ...
      if (this.ancestors > 0) {
        ...
        while (true) {
          if (ite.contains(head) && ite.index < head) {
            batch.addTreeNode(blankNode(ite.index))
          }
...
module.exports = class MerkleTree {
  ...
  static async open (dir, opts = {}) {
    const storage = new RocksStorage(dir)
    let foundLength = 0
    for await (const node of storage.iterator({ reverse: true })) {
      foundLength = node.length
      break
    }
...
async function getNodeSize (tree, index, batch) {
  const node = await tree.get(index, true, batch)
  return node.size
}
