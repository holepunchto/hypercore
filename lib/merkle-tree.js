...
class MerkleTreeBatch {
  ...
  appendRootSingle (node) {
    // we already have root
    if (node.index < (this.length - 1) * 2) return

    const ite = flat.iterator(node.index)

    for (let i = 0; i < this.roots.length; i++) {
      if (ite.contains(this.roots[i].index)) {
        while (i < this.roots.length) this.roots.pop()
        break
      }
    }
    ...
    // force length in case we added roots out of order
    this.length = ite.rightSpan() / 2 + 1
  }
...
module.exports = class MerkleTree {
  ...
  verifyUpgrade (proof, batch) {
    ...
    if (verifyUpgrade(proof, unverified, batch)) return batch
    return null
  }
