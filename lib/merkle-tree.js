...
class MerkleTreeBatch {
  ...
  get (index, error) {
    ...
    return MerkleTree.get(this.storage, index, error)
  }
  ...
  async restore (length) {
    ...
    const roots = unslabNodes(await MerkleTree.getRoots(this.storage, length))
...
class ReorgBatch extends MerkleTreeBatch {
  ...
  async _update (nodes) {
    ...
    while ((ite.index & 1) !== 0) {
      ...
      const existing = await MerkleTree.get(this.storage, left.index, false)
...
class ByteSeeker {
  ...
  async _seek (bytes) {
    ...
    for (const node of this.session.roots) { // all async ticks happen once we find the root so safe
      ...
      while ((ite.index & 1) !== 0) {
        const l = await MerkleTree.get(this.session.storage, ite.leftChild(), false)
...
class MerkleTree {
  ...
  static getRoots (storage, length) {
  ...
  static getNeededNodes (storage, length, start, end) {
    const nodes = new Map()
    const head = length * 2
    ...
    for (let i = start; i < end; i++) {
      ...
      while (true) {
        ...
        nodes.set(ite.index, this.get(storage, ite.index, true))
        ...
        nodes.set(sibling, this.get(storage, sibling, true))
      }
    }
  ...
  static async upgradeable (storage, length) {
    const indexes = flat.fullRoots(2 * length)
    const roots = new Array(indexes.length)
    const readBatch = storage.read()
  ...
  static get (storage, index, error = true, readBatch = null) {
    if (readBatch) return readBatch.getTreeNode(index, error)

    return getTreeNode(storage, index, error)
  }
  ...
  static async truncate (storage, length, batch, fork = batch.fork) {
    const head = length * 2
    ...
    for (let i = 0; i < fullRoots.length; i++) {
      ...
      batch.roots.push(unslabNode(await this.get(storage, root)))
    }
  ...
  static async reorg (storage, proof, batch) {
    let unverified = null
    ...
    for (const root of batch.roots) {
      const existing = await MerkleTree.get(storage, root.index, false)
  ...
  static verifyFullyRemote (proof, session) {
  ...
  static async verify (storage, proof, session) {
    const batch = new MerkleTreeBatch(session)
    ...
    if (unverified) {
      const verified = await MerkleTree.get(storage, unverified.index)
  ...
  static async missingNodes (storage, index, length) {
    const head = 2 * length
    ...
    while (!ite.contains(head) && !(await hasTreeNode(storage, ite.index))) {
      cnt++
      ite.parent()
    }
...
async function seekFromHead (session, head, bytes, padding) {
  ...
  for (let i = 0; i < roots.length; i++) {
    const root = roots[i]
    const node = await MerkleTree.get(session.storage, root, false)
...
async function seekTrustedTree (session, root, bytes, padding) {
  ...
  while ((ite.index & 1) !== 0) {
    const l = await MerkleTree.get(session.storage, ite.leftChild(), false)
...
function seekProof (session, batch, seekRoot, root, p) {
  ...
  p.seek = []
  p.seek.push(MerkleTree.get(session.storage, ite.index, true, batch))
...
function blockAndSeekProof (session, batch, node, seek, seekRoot, root, p) {
  ...
  p.node = []
  if (!node.value) p.node.push(MerkleTree.get(session.storage, ite.index, true, batch))
  ...
  while (ite.index !== root) {
    ...
    if (seek && ite.contains(seekRoot) && ite.index !== seekRoot) {
      ...
      p.node.push(MerkleTree.get(session.storage, ite.index, true, batch))
    }
...
function upgradeProof (session, batch, node, seek, from, to, subTree, p) {
  ...
  for (const ite = flat.iterator(0); ite.fullRoot(to); ite.nextTree()) {
    ...
    p.upgrade.push(MerkleTree.get(session.storage, ite.index, true, batch))
  }
}
  ...
  for (const ite = flat.iterator(0); ite.fullRoot(to); ite.nextTree()) {
    ...
    if (p.additionalUpgrade === null && ite.contains(from - 2)) {
      ...
      while (ite.index !== root) {
        ite.sibling()
        if (ite.index > target) {
          p.additionalUpgrade.push(MerkleTree.get(session.storage, ite.index, true, batch))
        }
        ite.parent()
      }
