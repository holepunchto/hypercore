...
module.exports = class MerkleTree {
  ...
  batch (session) {
    return new MerkleTreeBatch(this, session)
  }
  ...
  async truncate (length, session, fork = session.fork) {
    const head = length * 2
    const batch = new MerkleTreeBatch(this, session)
  ...
  async reorg (proof, session) {
    const batch = new ReorgBatch(this, session)
  ...
  proof (batch, session, { block, hash, seek, upgrade }) {
    return generateProof(batch, this.batch(session), block, hash, seek, upgrade)
  }
