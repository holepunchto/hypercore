...
class NodeQueue {
  ...
  shift (index) {
    ...
    if (this.i >= this.nodes.length) {
      throw new Error('Unexpected node: ' + index)
    }
...
class MerkleBatch {
  ...
  _verifyBlock (block) {
    ...
    const q = new NodeQueue(block.nodes, null)
...
module.exports = class MerkleTree {
  ...
  async nodes (index) {
    ...
    while (!(await this.has(ite.index)) && (ite.index + ite.factor / 2) < head) ite.parent()
    return ite.factor / 2 - 1
  }
...
function autoLength (storage) {
  return new Promise((resolve) => {
    storage.stat(function (err, st) {
      ...
      return resolve((nodes + (nodes & 1)) / 2)
    })
  })
}
