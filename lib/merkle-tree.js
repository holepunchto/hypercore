...
class MerkleTreeBatch {
  ...
  commit () {
    ...
    if (this.length < this.treeLength) {
      this.tree.truncateTo = this.tree.truncated
        ? Math.min(this.tree.truncateTo, this.length)
        : this.length

      this.tree.truncated = true
      truncateMap(this.tree.unflushed, this.length)
      if (this.tree.flushing !== null) truncateMap(this.tree.flushing, this.length)
    }
...
class ReorgBatch extends MerkleTreeBatch {
  ...
  commit () {
    // TODO: rework this as it's just a dup of the above truncation logic
    if (this.ancestors > 0 && this.ancestors < this.tree.length) {
      ...
      while (true) {
        if (ite.contains(head) && ite.index < head) {
          blanks.push(blankNode(ite.index))
        }
        if (ite.offset === 0) break
        ite.parent()
      }

      this.nodes = blanks.concat(this.nodes)
...
module.exports = class MerkleTree extends Flushable {
  ...
  get (index, error = true) {
    ...
    // TODO: test this
    if (this.truncated && node.index >= 2 * this.truncateTo) {
  ...
  async truncate (length, opts = { fork: this.fork }) {
    ...
    if (length) {
      ...
      while (true) {
        if (ite.contains(head) && ite.index < head) {
          batch.nodes.push(blankNode(ite.index))
        }
        if (ite.offset === 0) break
        ite.parent()
      }
  ...
  static async open (storage, opts = {}) {
    ...
    return new MerkleTree(storage, roots, opts)
  }
}
