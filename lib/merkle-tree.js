...
class MerkleTreeBatch {
  ...
  async downgrade() {
    ...
    const rx = this.storage.read(this.fork)
    const nodePromises = []
...
class MerkleTree {
  ...
  static async upgradeable(session, length) {
    const indexes = flat.fullRoots(2 * length)
    const roots = new Array(indexes.length)
    const rx = session.storage.read(session.fork)
...
async function getByteOffsetSession(session, index, rx) {
  ...
  const treeNodes =
    rx === null
      ? await getByteOffsetBatchFlush(session.roots, index, session.storage.read(session.fork))
