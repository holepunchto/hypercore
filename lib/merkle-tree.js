...
class MerkleTreeBatch {
  ...
  commit (writeBatch) {
    if (writeBatch === undefined) throw INVALID_OPERATION('No database batch was passed')
    if (!this.commitable()) throw INVALID_OPERATION('Tree was modified during batch, refusing to commit')

    if (this.upgraded) this._commitUpgrade(writeBatch)
...
module.exports = class MerkleTree {
  ...
  clear (writeBatch) {
    const wb = writeBatch || this.storage.createWriteBatch()
...
async function generateProof (tree, block, hash, seek, upgrade) {
  ...
  const readBatch = tree.storage.createReadBatch()
  ...
  if (node !== null && (!upgrade || node.lastIndex < upgrade.start)) {
    ...
    blockAndSeekProof(tree, readBatch, node, seek, seekRoot, subTree, p)
  } else if ((node || seek) && upgrade) {
    subTree = seek ? await seekFromHead(tree, to, seek.bytes, seek.padding) : node.index
  }

  if (upgrade) {
    upgradeProof(tree, readBatch, node, seek, from, to, subTree, p)
    if (head > to) additionalUpgradeProof(tree, readBatch, to, head, p)
  }
