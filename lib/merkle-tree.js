...
class MerkleTreeBatch {
  ...
  commit () {
    ...
    const batch = this.tree.storage.createWriteBatch()

    if (this.upgraded) this._commitUpgrade(batch)

    for (let i = 0; i < this.nodes.length; i++) {
      const node = this.nodes[i]
      batch.addTreeNode(this.batchId, node)
    }

    return batch.flush()
  }

  _commitUpgrade (batch) {
    ...
    if (this.ancestors < this.treeLength) {
      batch.deleteTreeNodeRange(this.batchId, this.ancestors, this.treeLengthq)
  ...
  byteRange (index, batch = null) {
    return getByteRange(this, index, batch)
  }

  byteOffset (index, batch = null) {
    if (index === 2 * this.tree.length) return this.tree.byteLength
    return getByteOffset(this, index, batch)
  }
}
...
module.exports = class MerkleTree {
  ...
  getRoots (length) {
    ...
    for (let i = 0; i < indexes.length; i++) {
      roots[i] = batch.getTreeNode(this.batchId, indexes[i], true)
    }
  ...
  async upgradeable (length) {
    ...
    for (let i = 0; i < indexes.length; i++) {
      roots[i] = batch.getTreeNode(this.batchId, indexes[i], false)
    }
  ...
  get (index, error = true, batch = null) {
    if (batch) return batch.getTreeNode(this.batchId, index, error)

    return this.storage.getTreeNode(this.batchId, index, error)
  }

  clear () {
    const batch = this.storage.createWriteBatch()
    batch.deleteTreeNodeRange(this.batchId, 0, this.length)
...
async function getNodeSize (tree, index, batch) {
  return (await batch.getTreeNode(tree.batchId, index, true)).size
}
  ...
  for (const node of tree.roots) { // all async ticks happen once we find the root so safe
    ...
    while (ite.index !== index) {
      if (index < ite.index) {
        ...
        promises.push(tree.get(ite.leftChild(), true, batch))
        ite.sibling()
      }
    }
...
function getByteRange (tree, index, batch) {
  ...
  const offset = tree.byteOffset(index, batch)
  const size = getNodeSize(tree, index, batch)
...
async function seekFromHead (tree, batch, head, bytes, padding) {
  ...
  for (let i = 0; i < roots.length; i++) {
    ...
    return seekTrustedTree(tree, batch, root, bytes, padding)
  }
...
async function seekTrustedTree (tree, batch, root, bytes, padding) {
...
async function seekUntrustedTree (tree, batch, root, bytes, padding) {
...
async function generateProof (tree, block, hash, seek, upgrade) {
  ...
  if (node !== null && (!upgrade || node.lastIndex < upgrade.start)) {
    ...
    const seekRoot = seek ? await seekUntrustedTree(tree, null, subTree, seek.bytes, seek.padding) : head
    blockAndSeekProof(tree, batch, node, seek, seekRoot, subTree, p)
  } else if ((node || seek) && upgrade) {
    subTree = seek ? await seekFromHead(tree, null, to, seek.bytes, seek.padding) : node.index
  }

  if (upgrade) {
    upgradeProof(tree, batch, node, seek, from, to, subTree, p)
    if (head > to) additionalUpgradeProof(tree, batch, to, head, p)
  }
