...
class MerkleTreeBatch {
  ...
  commit () {
    ...
    const dbBatch = this.tree.storage.createWriteBatch()

    if (this.upgraded) this._commitUpgrade(dbBatch)

    for (let i = 0; i < this.nodes.length; i++) {
      const node = this.nodes[i]
      dbBatch.addTreeNode(this.batchId, node)
    }

    return dbBatch.flush()
  }

  _commitUpgrade (dbBatch) {
    ...
    if (this.ancestors < this.treeLength) {
      dbBatch.deleteTreeNodeRange(this.batchId, this.ancestors, this.treeLengthq)
  ...
  byteRange (index, dbBatch = null) {
    return getByteRange(this, index, dbBatch)
  }

  byteOffset (index, dbBatch = null) {
    if (index === 2 * this.tree.length) return this.tree.byteLength
    return getByteOffset(this, index, dbBatch)
  }
}
...
module.exports = class MerkleTree {
  ...
  getRoots (length) {
    ...
    const dbBatch = this.storage.createReadBatch()

    for (let i = 0; i < indexes.length; i++) {
      roots[i] = dbBatch.getTreeNode(this.batchId, indexes[i], true)
    }
  ...
  async upgradeable (length) {
    ...
    for (let i = 0; i < indexes.length; i++) {
      roots[i] = dbBatch.getTreeNode(this.batchId, indexes[i], false)
    }
  ...
  get (index, error = true, dbBatch = null) {
    if (dbBatch) return dbBatch.getTreeNode(this.batchId, index, error)

    return this.storage.getTreeNode(this.batchId, index, error)
  }

  clear () {
    const dbBatch = this.storage.createWriteBatch()
    dbBatch.deleteTreeNodeRange(this.batchId, 0, this.length)
...
async function getNodeSize (tree, index, dbBatch) {
  return (await dbBatch.getTreeNode(tree.batchId, index, true)).size
}

async function getByteOffset (tree, index, dbBatch) {
  ...
  for (const node of tree.roots) { // all async ticks happen once we find the root so safe
    ...
    if (dbBatch === null) dbBatch = tree.storage.createReadBatch()
    const promises = []
    ...
    while (ite.index !== index) {
      if (index < ite.index) {
        ...
        promises.push(tree.get(ite.leftChild(), true, dbBatch))
        ite.sibling()
      }
    }
...
function getByteRange (tree, index, dbBatch) {
  ...
  const offset = tree.byteOffset(index, dbBatch)
  const size = getNodeSize(tree, index, dbBatch)
...
async function generateProof (tree, block, hash, seek, upgrade) {
  ...
  const dbBatch = tree.storage.createReadBatch()
  ...
  if (node !== null && (!upgrade || node.lastIndex < upgrade.start)) {
    ...
    blockAndSeekProof(tree, dbBatch, node, seek, seekRoot, subTree, p)
  } else if ((node || seek) && upgrade) {
    subTree = seek ? await seekFromHead(tree, to, seek.bytes, seek.padding) : node.index
  }

  if (upgrade) {
    upgradeProof(tree, dbBatch, node, seek, from, to, subTree, p)
    if (head > to) additionalUpgradeProof(tree, dbBatch, to, head, p)
  }
