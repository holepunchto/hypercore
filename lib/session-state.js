...
module.exports = class SessionState {
  ...
  _commit () {
    ...
    this._pendingBitfield = null
    this.parent._oncommit(this, bitfield)
  }

  _oncommit (src, bitfield) {
    this.fork = src.fork
    ...
    if (!bitfield || !bitfield.drop) {
      ...
      this.ontruncate(tree, bitfiled.start, bitfield.start + bitfield.length, true)
    }
  }
  ...
  async _verifyBlock (batch, bitfield, value, manifest, from) {
    ...
    try {
      ...
      if (batch.upgraded) this.onappend(tree, bitfield, flushed)
    } finally {
      this._clearActiveBatch()
      this.updating = false
      lock.release()
  ...
  ontruncate (tree, to, from, flushed) {
    const bitifeld = { start: to, length: from - to, drop: true }
  ...
  _updateBitfield (bitfield, flushed) {
    ...
    if (p === null) {
      this._pendingBitfield = bitfield
      return
    }
  ...
  async moveTo (core, length) {
    ...
    try {
      ...
      if (!this.isSnapshot()) {
        ...
        if (truncation) {
          const { dependency, tree, flushing } = truncation
  ...
  async createSession (name, length, overwrite, atom) {
    ...
    if (!overwrite && this.storage) {
      storage = await this.storage.resumeBatch(name)
    ...
    if (storage === null) {
      ...
      storage = await this.storage.createBatch(name, treeInfo, atom)
    }
    ...
    const state = new SessionState(this.core, atom ? this : null, storage, this.core.blocks, tree, head, name)
