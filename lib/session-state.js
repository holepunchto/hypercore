...
class SessionState {
  ...
  async clear(start, end, cleared) {
    ...
    try {
      ...
      const flushedLength = this.flushedLength()
      const dependency = start < flushedLength ? updateDependency(this, start, true) : null

      // Dependency was reset but the end doesn't include all blocks
      if (dependency && end < flushedLength - 1) {
        const blocksPromises = []
        const rx = this.storage.read()
        for (let i = end + 1; i < flushedLength; i++) {
          blocksPromises.push(rx.getBlock(i))
        }
        rx.tryFlush()
        const blocks = await Promise.all(blocksPromises)
        let j = 0
        for (let i = end + 1; i < flushedLength; i++) {
          tx.putBlock(i, blocks[j])
          j++
        }
      }
