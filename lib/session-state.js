...
module.exports = class SessionState {
  ...
  signedLength() {
    ...
    return this.isSnapshot() && l > this.snapshotCompatLength
  ...
  async updateSnapshotStorage(storage) {
    ...
    rx.tryFlush()
    const [head, auth, dep] = await Promise.all([
      headPromise,
      authPromise,
      depPromise
    ])
  ...
  async _oncommit(src, bitfield) {
    ...
    try {
      ...
      const append = b
        ? { start: b.start, length: b.appends, drop: false }
        : null
  ...
  async _truncate(storage, batch) {
    ...
    const dependency = truncated
      ? updateDependency(this, batch.length, true)
      : null
  ...
  async clear(start, end, cleared) {
    ...
    try {
      ...
      while (!isRootIndex(ite.index, this.roots)) {
        ...
        const e = right / 2 + 1
        const has =
          s <= start && e <= end ? false : this.core.bitfield.hasSet(s, e - s)
        if (has) break
      ...
      const dependency =
        start < this.flushedLength()
          ? updateDependency(this, start, true)
          : null
  ...
  async append(values, { signature, keyPair, preappend, maxLength = -1 } = {}) {
    ...
    try {
      ...
      if (this.prologue && batch.length < this.prologue.length) {
        throw INVALID_OPERATION(
          'Append is not consistent with prologue',
          this.core.discoveryKey
        )
      }
  ...
  _updateBitfield(bitfield, flushed) {
    ...
    if (b.drop) {
      ...
      if (p && b.start + b.length !== p.start + p.appends) {
        throw INVALID_OPERATION(
          'Atomic truncations must be contiguous',
          this.core.discoveryKey
        )
      }
    ...
    if (b.start !== p.start + p.appends) {
      throw INVALID_OPERATION(
        'Atomic operations must be contiguous',
        this.core.discoveryKey
      )
    }
  ...
  async commit(
    state,
    { signature, keyPair, length = state.length, treeLength = -1 } = {}
  ) {
    assert(
  ...
  async moveTo(core, length) {
    ...
    try {
      ...
      if (!this.isSnapshot()) {
        ...
        const truncation =
          length < this.length ? await truncateAndFlush(this, length) : null
