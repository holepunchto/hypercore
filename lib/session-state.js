...
module.exports = class SessionState {
  ...
  async _oncommit (src, bitfield) {
    ...
    try {
      ...
      if (bitfield && bitfield.drop && bitfield.start < currLength) {
  ...
  _updateBitfield (bitfield, flushed) {
    ...
    if (b.drop) {
      ...
      if (p === null || b.start < p.start) {
        this._pendingBitfield = { drop: true, start: b.start, length: 0 }
        return
      }
    ...
    if (p === null) {
      this._pendingBitfield = { drop: false, start: b.start, length: b.length }
      return
    }
