...
module.exports = class SessionState {
  constructor (core, parent, storage, treeInfo, name, trace, length) {
    this.core = core
    ...
    if (this.roots.includes(null)) {
      ...
      const getBlock = (index) => {
        const p = []
        const rx = this.storage.read()
        if (index > 0) p.push(rx.getBlock(index - 1))
        p.push(rx.getBlock(index))
        rx.tryFlush()
        return Promise.all(p)
      }

      const getTreeNode = (index) => {
        const rx = this.storage.read()
        const p = rx.getTreeNode(index)
        rx.tryFlush()
        return p
      }

      const index = treeInfo ? treeInfo.length - 1 : 0
      const treeIndex = flat.fullRoots(treeInfo.length * 2).pop()

      getTreeNode(treeIndex).then(treeNode => {
        getBlock(index).then(blocks => {
          getHead().then(head => {
            const referrer = this.core.header.userData.find(u => u.key === 'referrer')
            console.log('SESSION HAS NULL ROOTS')
            console.log('  KEY:', this.core.header.key.toString('hex'))
            console.log('  LENGTH:', treeInfo.length)
            console.log('  ROOTS', this.roots.map(r => r ? r.index : 'null').join(' '))
            console.log('  EXP ROOTS:', flat.fullRoots(treeInfo.length * 2).join(' '))
            console.log('  DEPENDENCIES:')
            console.log('  BITFIELD:', this.core.bitfield.get(index))
            for (const dep of this.storage.dependencies) {
              console.log(`    data: ${dep.dataPointer} -> length: ${dep.length}`)
            }
            console.log('  REFERRER:', this.core.header.userData.find(u => u.key === 'referrer'))
            console.log('  PROLOGUE:', treeInfo.prologue?.length || null)
            console.log('  HEAD', head)
            console.log('  TREE NODE', treeNode)
            console.log('  BLOCK AT ' + (index - 1) + ':', !!blocks[0], block[0])
            console.log('  BLOCK AT ' + index + ':', !!blocks[1], block[1])
            console.log('  CREATED AT:', trace)
            console.log('  CORE:', this.core)
            console.log('  SESSION:', this)

            if (global.hook) global.hook(referrer.value)
          })
        })
      })
    }
  ...
  async createSession (name, overwrite, atom, trace) {
    let storage = null
    let treeInfo = null
