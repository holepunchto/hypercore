...
module.exports = class SessionState {
  ...
  async _oncommit (src, bitfield) {
    ...
    if (src.flushedLength > this.flushedLength) {
      this.storage.updateDependencyLength(src.flushedLength)
    }
  ...
  async truncate (length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      if (dependency) this.storage.updateDependencyLength(dependency.length)
  ...
  async commit (state, { signature, keyPair, length = state.length, treeLength = state.flushedLength(), overwrite = false } = {}) {
    ...
    try {
      ...
      if (treeLength < length) {
        ...
        if (dependency) state.storage.updateDependencyLength(dependency.length)
      }
