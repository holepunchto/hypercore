...
module.exports = class SessionState {
  ...
  async _oncommit (src, bitfield) {
    ...
    try {
      ...
      if (bitfield && bitfield.truncated && bitfield.start < currLength) {
        this.ontruncate(tree, bitfield.start, currLength, true)
  ...
  _updateBitfield (bitfield, flushed) {
    ...
    if (b.drop) {
      ...
      if (p && (b.start + b.length !== p.start + p.length)) {
        throw INVALID_OPERATION('Atomic truncations must be contiguous')
      }

      // actual truncation
      if (p === null || b.start < p.start) {
        this._pendingBitfield = { truncated: true, start: b.start, length: 0 }
        return
      }
    ...
    if (p === null) {
      this._pendingBitfield = { truncated: false, start: b.start, length: b.length }
      return
    }
