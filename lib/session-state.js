...
class SessionState {
  ...
  async _oncommit(src, bitfield) {
    ...
    try {
      ...
      if (this.isDefault() && this.core.header.group) {
        ...
        tx.deleteGroupUpdate(pointer, this.core.header.timestamp)
    ...
    try {
      ...
      if (batch.upgraded) {
        ...
        if (this.core.header.group) {
          ...
          tx.putGroupUpdate(pointer, head.timestamp, this.core.header.key)
        }
      }
  ...
  async _truncate(storage, batch) {
    ...
    if (this.isDefault() && this.core.header.group) {
      ...
      storage.deleteGroupUpdate(pointer, this.core.header.timestamp)
      storage.putGroupUpdate(pointer, tree.timestamp, this.core.header.key)
    }
