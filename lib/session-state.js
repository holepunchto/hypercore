...
module.exports = class SessionState {
  ...
  async _oncommit(src, bitfield) {
    ...
    try {
      ...
      if (b && b.truncated && b.start < currLength) {
        await this.ontruncate(tree, b.start, currLength, true)
        if (!b || b.appends === 0) return
      }

      const append = b ? { start: b.start, length: b.appends, drop: false } : null

      await this.onappend(tree, append, true)
    } finally {
      this.mutex.unlock()
      this.core.checkIfIdle()
    }
  ...
  async _verifyBlock(batch, bitfield, value, manifest, from) {
    ...
    try {
      ...
      if (manifest) await this.core._setManifest(tx, manifest, null)
      ...
      if (batch.upgraded) {
        ...
        await this.onappend(head, bitfield, flushed)
      }
    } finally {
      this._clearActiveBatch()
      this.updating = false
  ...
  async truncate(length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      if (this.core.verifier === null && keyPair) await this.core._setManifest(tx, null, keyPair)
      ...
      await this.ontruncate(tree, tree.length, batch.treeLength, flushed)
    } finally {
      this._unlock()
    }
  }
  ...
  async reorg(batch) {
    ...
    try {
      ...
      await this.ontruncate(tree, batch.ancestors, batch.treeLength, flushed)
    } finally {
      this._unlock()
    }
  }
  ...
  async clear(start, end, cleared) {
    ...
    try {
      ...
      while (!isRootIndex(ite.index, this.roots)) {
        ...
        const e = right / 2 + 1
        const has =
          s <= start && e <= end ? false : await this.core.replicator.localBitfield.hasSet(s, e - s)
        if (has) break
      ...
      if (this.isDefault() && flushed) {
        ...
        await this.core.replicator.onhave(start, length, true)
      }
    } finally {
      this._unlock()
    }
  ...
  async append(values, { signature, keyPair, preappend, maxLength = -1 } = {}) {
    ...
    try {
      ...
      await this.onappend(tree, bitfield, flushed)
  ...
  async onappend(tree, bitfield, flushed) {
    if (!flushed) this._updateBitfield(bitfield)
    else if (this.isDefault()) await this.core.onappend(tree, bitfield)
  ...
  async ontruncate(tree, to, from, flushed) {
    const bitfield = { start: to, length: from - to, drop: true }

    this.lastTruncation = { from, to }

    if (!flushed) this._updateBitfield(bitfield)
    else if (this.isDefault()) await this.core.ontruncate(tree, bitfield)
  ...
  async catchup(length) {
    ...
    try {
      ...
      if (truncating) await this.ontruncate(tree, sharedLength, origLength, flushed)
      if (sharedLength < length) await this.onappend(tree, null, flushed)
    } finally {
      this.mutex.unlock()
    }
  }
