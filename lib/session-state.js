...
module.exports = class SessionState {
  constructor (core, parent, storage, blocks, treeInfo, name) {
    this.core = core
  ...
  async truncate (length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      await MerkleTree.truncate(this.storage, length, batch, fork)
  ...
  async _getTreeHeadAt (length) {
    ...
    const roots = await MerkleTree.getRoots(this.storage, length)
  ...
  async moveTo (core, length) {
    ...
    try {
      ...
      if (!this.isSnapshot()) {
        ...
        this.fork = fork
        this.length = length
        this.roots = await MerkleTree.getRoots(this.storage, length)
  ...
  async createSession (name, overwrite, atom) {
    ...
    const head = {
      fork: this.fork,
      roots: length === this.length ? this.roots.slice() : await MerkleTree.getRoots(storage, length),
      length,
      prologue: this.prologue,
...
async function truncateAndFlush (s, length) {
  ...
  await MerkleTree.truncate(s.storage, length, batch, s.fork)
