...
class SessionState {
  ...
  async _precommit(state) {
    assert(state.parent === this &&
      state.storage.core.dataPointer === this.storage.core.dataPointer,
  ...
  static isParent(state, parent) {
    ...
    if (state.parent) return state.parent === parent
  ...
  async createSession(name, overwrite, atom) {
    ...
    try {
      ...
      if (atom) {
        this.atomized = atom
        atom.onflush(this._precommit.bind(this, state))
