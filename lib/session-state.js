...
module.exports = class SessionState {
  ...
  log (...msg) {
    console.log('[session-state' + this.clock + ']', 'dep=' + this.dependencyLength, ...msg)
  }
  ...
  flushedLength () {
    ...
    if (this.dependencyLength === 0) {
      this.log('flushedLength returning 0')
    }
  ...
  updateDependency (tx, length) {
    ...
    if (dependency) {
      this.log('updateDependency', length)
  ...
  async _truncate (storage, batch) {
    ...
    if (dependency) {
      this.log('_truncate', dependency.length)
      this.dependencyLength = dependency.length
    }
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      if (dependency) {
        this.log('_clear', dependency.length)
        this.dependencyLength = dependency.length
      }
  ...
  async _overwrite (source, fork, length, treeLength, signature, isDependent, shallow) {
    ...
    if (dependency) {
      this.log('_overwrite', dependency.length)
      this.dependencyLength = dependency.length
    }
    ...
    try {
      ...
      if (treeLength < length) {
        ...
        if (dependency) {
          ...
          this.log('commit', dependency.length)
  ...
  _moveToCore (core) {
    ...
    // storage was updated
    const deps = this.storage.dependencies
    this.log('_moveToCore', deps[deps.length - 1].length)
