...
module.exports = class SessionState {
  ...
  async _overwrite (source, fork, length, treeLength, signature, isDependent, infill, atom) {
    const blockPromises = []
    const treePromises = []
    const rootPromises = []
  ...
  async overwrite (state, { length = state.tree.length, treeLength = state.flushedLength(), infill = true, atom } = {}) {
    ...
    try {
      ...
      const tree = await this._overwrite(state, fork, length, treeLength, null, state === this.core.state, infill, atom)
  ...
  async moveTo (core, length, { atom } = {}) {
    ...
    try {
      ...
      if (!this.isSnapshot()) {
        ...
        treeInfo.fork = truncation ? this.tree.fork + 1 : this.tree.fork
        ...
        newStorage = await registration
        newTree = await MerkleTree.open(newStorage, treeInfo.length, { prologue, fork: treeInfo.fork })
...
async function truncateAndFlush (s, length, atom) {
  const batch = await s.tree.truncate(length, s.tree.fork)
