...
async function flushBatch(prologue, src, dst, batch) {
  ...
  for (const { index } of batch.blocks) {
    if (start === 0 || start - 1 === index) {
      ...
      if (length > 0) await signalReplicator(dst, upgraded, start, length)
      upgraded = false
      length = 1
    }
...
async function signalReplicator(core, upgraded, start, length) {
  if (upgraded) {
    core.replicator.cork()
    await core.replicator.onhave(start, length, false)
    await core.replicator.onupgrade()
