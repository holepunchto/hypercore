...
module.exports = class DefaultEncryption {
  ...
  static deriveKeys(
    encryptionKey,
    hypercoreKey,
    { block = false, compat = false } = {}
  ) {
    const subKeys = b4a.alloc(2 * sodium.crypto_stream_KEYBYTES)

    const blockKey = block
      ? encryptionKey
      : subKeys.subarray(0, sodium.crypto_stream_KEYBYTES)
    ...
    if (!block) {
      if (compat)
        ...
        sodium.crypto_generichash_batch(blockKey, [
          DEFAULT_ENCRYPTION,
          hypercoreKey,
          encryptionKey
        ])
  ...
  encrypt(index, block, fork, core) {
    ...
    return DefaultEncryption.encrypt(
      index,
      block,
      fork,
      this.blockKey,
  ...
  _reload(core) {
    ...
    const keys = DefaultEncryption.deriveKeys(this.key, core.key, {
      block,
      compat: core.compat
    })
