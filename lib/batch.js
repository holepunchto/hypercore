...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  verify (length, signature, publicKey) {
    const s = this.createTreeBatch(null, length)
    if (s === null) return false
    return this.core.tree.crypto.verify(s.signable(), signature, publicKey)
  }
  ...
  createTreeBatch (blocks, length = this._appends.length + (blocks ? blocks.length : 0)) {
    if (length > this._appends.length + (blocks ? blocks.length : 0)) {
      return null
    }

    const b = this.session.createTreeBatch()
    const len = Math.min(length, this._appends.length)
