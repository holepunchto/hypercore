const { SESSION_NOT_WRITABLE, BLOCK_NOT_AVAILABLE } = require('hypercore-errors')
...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  verify (length, signature, publicKey) {
    const s = this.signable(null, length)
    return this.core.tree.crypto.verify(s, signature, publicKey)
  }
  ...
  async _flush (length, auth) { // TODO: make this safe to interact with a parallel truncate...
    ...
    const flushingByteLength = this._byteLength

    await this.session.append(flushingLength < this._appends.length ? this._appends.slice(0, flushingLength) : this._appends, auth)

    this._appends = this._appends.slice(flushingLength)
    this._sessionLength += flushingLength
    this._sessionByteLength += flushingByteLength
