...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  async flush (opts = {}) {
    ...
    const { length = this.length, keyPair = this.session.keyPair, signature = null, pending = false } = opts
  ...
  async _flush (length, keyPair, signature, pending) { // TODO: make this safe to interact with a parallel truncate...
    ...
    const info = await this.core.insertBatch(batch, this._appendsActual, { keyPair, signature, pending })
