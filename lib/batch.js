...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  createTreeBatch (blocks, length = this.length + (blocks ? blocks.length : 0)) {
    const maxLength = this.length + (blocks ? blocks.length : 0)
    ...
    for (let i = 0; i < len; i++) {
      b.append(this._appends[i - this._sessionLength])
    }

    if (len < this.length) return b

    for (let i = len; i < length; i++) {
      b.append(blocks[i - len - this._sessionLength])
    }
