...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  async _catchup () {
    ...
    for (let i = 0; i < offset; i++) {
      await this._sessionBatch.append(this._appends[i])
      this._byteLength -= this._appends[i]
    }

    await this.session.core._fillBlocksUnsafe(this._appends.slice(0, offset), this._sessionLength, this._sessionByteLength, this._sessionBatch.nodes)
