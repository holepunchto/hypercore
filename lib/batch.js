...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  async flush (length = this._appends.length, signature) {
    if (this.opened === false) await this.opening
    if (this.closing) throw SESSION_CLOSED()

    while (this._flushing) await this._flushing
    this._flushing = this._flush(length, signature)
  ...
  async _flush (length, signature) { // TODO: make this safe to interact with a parallel truncate...
    if (this._appends.length === 0) return

    const flushingLength = Math.min(length, this._appends.length)
    const info = await this.session.append(flushingLength < this._appends.length ? this._appends.slice(0, flushingLength) : this._appends, { signature })
