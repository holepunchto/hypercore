...
module.exports = class HypercoreBatch extends EventEmitter {
  ...
  async _flush (length, keyPair, signature) { // TODO: make this safe to interact with a parallel truncate...
    ...
    if (this.session.length > this._sessionLength) {
      await this._catchupFlush(flushingLength)

      if (this._sessionLength === expectedLength) {
        this.emit('flush')
        return false
      }
    }
  ...
  async _catchupFlush (length, auth) { // todo: better name
    let delta = 0
    const stop = Math.min(this.session.length - this._sessionLength, length)

    for (let i = 0; i < stop; i++) {
      const block = await this.session.get(this._sessionLength + i)
      const local = this._appends[i]

      delta += local.byteLength // todo, not correct
      this.treeBatch.append(block)
    }

    this._sessionLength += stop
    this._sessionByteLength += delta
    this._appends = this._appends.slice(stop)
    this._byteLength -= delta
  }
