...
class LocalWants {
  ...
  addAnyRange(start, length, handle) {
    if (this.anys === null) this.anys = []

    for (let i = 0; i < this.anys.length; i++) {
      const w = this.anys[i]
    ...
    this.anys.push({ wants: this, start, length, any: true, handles: new Set([handle]) })
    return { start, length, any: true }
  }

  removeAnyRange(start, length, handle) {
    if (this.anys === null) return null
    ...
    for (let i = 0; i < this.anys.length; i++) {
      ...
      const head = this.anys.pop()
      if (head !== w) this.anys[i] = head
      return { start, length }
    }
...
class RemoteWants {
  ...
  add(range) {
    if (range.any) {
      ...
      if (this.any.length < MAX_ANY) this.anys.push(range)
      else this.all = true
      return true
    }
