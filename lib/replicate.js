...
function Peer (feed, opts) {
  if (opts.extensions) throw new Error('Per peer extensions is deprecated. Use feed.registerExtension instead')
...
Peer.prototype._updateOptions = function () {
  if (this.ack || (this.feed.extensions.length)) {
    this.stream.options({
      ack: this.ack,
      extensions: this.feed.extensions.map(ext => ext.name)
    })
  }
}
...
Peer.prototype.onextension = function (id, message) {
  ...
  for (const ext of this.feed.extensions) {
    if (ext.name === name) return ext.onmessage(message, this)
  }
}
...
Peer.prototype._autoEnd = function () {
  ...
  if ((this.sparse || this.live) && this.remoteUploading) return
  this.end()
}
