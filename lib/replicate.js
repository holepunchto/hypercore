...
function Peer (feed, opts) {
  this.feed = feed
  this._id = feed.key.toString('hex').substring(0, 20)
...
Peer.prototype.onwant = function (want) {
  ...
  if (!this.remoteWant && this.feed.length && this.feed.bitfield.get(this.feed.length - 1)) {
    ...
    have = { start: this.feed.length - 1 }
    debug(this._id, 'sent HAVE', have)
    this.stream.have(have)
  }
  this.remoteWant = true
  var rle = this.feed.bitfield.compress(want.start, want.length)
  have = {start: want.start, length: want.length, bitfield: rle}
  debug(this._id, 'sent HAVE', have)
  this.stream.have(have)
}

Peer.prototype.ondata = function (data) {
  debug(this._id, 'recv DATA', data)
  var self = this
  ...
  this.feed._putBuffer(data.index, data.value, data, this, function (err) {
    ...
    if (self.remoteAck) {
      ...
      var have = {start: data.index, length: 1, ack: true}
      debug(self._id, 'sent HAVE', have)
      self.stream.have(have)
    }
...
Peer.prototype.onrequest = function (request) {
  debug(this._id, 'recv REQUEST', request)
...
Peer.prototype.onunhave = function (unhave) {
  debug(this._id, 'recv UNHAVE', unhave)
...
Peer.prototype.unhave = function (unhave) { // called by feed
  debug(this._id, 'send UNHAVE', unhave)
...
Peer.prototype._sendWant = function (index) {
  ...
  this.inflightWants++
  var want = {start: j * len, length: len}
  debug(this._id, 'sent WANT', want)
  this.stream.want(want)
  return true
}
