...
Peer.prototype.ontick = function () {
  ...
  if (first.hash ? this.feed.tree.get(first.index << 1) : this.feed.bitfield.get(first.index)) {
...
Peer.prototype._update = function () {
  ...
  while (inflight < this.maxRequests) {
    offset = (Math.random() * waiting.length) | 0
  ...
  while (inflight < this.maxRequests) {
    offset = (Math.random() * selections.length) | 0
...
Peer.prototype._downloadRange = function (range) {
  ...
  while (!this.remoteBitfield.get(i) || (range.hash && this.feed.tree.get(i << 1)) || !reserved.set(i, true)) {
    i = ite.next()
    reset = true
    ...
    if (range.hash) {
      ...
      for (var j = range.start; j < wantedEnd; j++) {
        if (!this.feed.tree.get(j << 1)) return
      }
      if (!range.bytes) {
        set.remove(this.feed._selections, range)
        range.callback(null)
