...
function replicate (feed, stream) {
  ...
  feed.ready(function (err) {
    ...
    peer.channel.have({
      start: 0,
      bitfield: feed.bitfield.compress()
    })
...
Peer.prototype.update = function () {
  // TODO: in the future if we support unlinking local cached data you
  // might still wanna download data, *even* if you are writable
  if (this.feed.writable) return
...
function onhave (message) {
  ...
  if (message.bitfield) { // TODO: support .start
    ...
    for (var i = 0; i < bitfield.length; i += pageSize) {
      peer.remoteBitfield.pages.set(i / pageSize, bitfield.slice(i, i + pageSize))
    }
  } else {
