...
function replicate (feed, opts) {
  ...
  function onready (err) {
    ...
    stream.on('close', continueIfUnwaited)
    stream.on('end', continueIfUnwaited)
    stream.on('error', continueIfUnwaited)
    var firstTime = true
    ...
    function continueIfUnwaited () {
      if (!unwaited) feed.ifAvailable.continue()
      unwaited = true
    }
  }
}
