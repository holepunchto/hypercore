...
const rle = require('bitfield-rle').align(4)
const treeIndex = require('./tree-index')
...
function replicate (feed, initiator, opts) {
  ...
  let stream = Protocol.isProtocolStream(initiator) ? initiator : opts.stream
...
Peer.prototype.onwant = function (want) {
  ...
  const rle = this.feed.bitfield.compress(want.start, want.length)
...
Peer.prototype.ondata = function (data) {
  ...
  const allowPush = this.feed.allowPush || !data.value
...
Peer.prototype.onrequest = function (request) {
  ...
  const self = this
  const opts = { digest: request.nodes, hash: request.hash, tree: this.remoteTree }
...
Peer.prototype._onrequesttimeout = function () {
  ...
  const first = this.inflightRequests[0]
...
Peer.prototype.onhave = function (have) {
  ...
  if (have.bitfield) { // TODO: handle start !== 0
    ...
    const buf = rle.decode(have.bitfield)
...
Peer.prototype._updateEnd = function () {
  ...
  const sel = this.feed._selections[0]
  let remoteLength = this.feed.length || -1

  for (let i = 0; i < this.feed.peers.length; i++) {
...
Peer.prototype.onstatus = function (info) {
  ...
  if (!info.uploading) {
    ...
    for (let i = 0; i < this.inflightWants; i++) {
      this.feed.ifAvailable.continue()
    }
    this.inflightWants = 0
    this.wants = bitfield()
...
Peer.prototype.onunhave = function (unhave) {
  let start = unhave.start
  let len = unhave.length || 1
...
Peer.prototype._update = function () {
  ...
  const selections = this.feed._selections
  const waiting = this.feed._waiting
  const wlen = waiting.length
  const slen = selections.length
  let inflight = this.inflightRequests.length
  let offset = 0
  let i = 0
...
Peer.prototype._sendWants = function () {
  ...
  for (i = 0; i < this.feed._selections.length; i++) {
    const s = this.feed._selections[i]
...
Peer.prototype._sendWantRange = function (s) {
  ...
  let want = s.start ? 1024 * 1024 * Math.floor(s.start / 1024 / 1024) : 0
...
Peer.prototype._downloadBlocks = function (range) {
  ...
  for (let i = range.blocksDownloaded; i < range.blocks.length; i++) {
...
Peer.prototype._downloadRange = function (range) {
  ...
  const reserved = this.feed._reserved
  const ite = this._iterator
  const wantedEnd = Math.min(range.end === -1 ? this.remoteLength : range.end, this.remoteLength)

  let i = range.linear ? ite.seek(range.start).next(true) : nextRandom(ite, range.start, wantedEnd)
  let start = i
  ...
  while ((range.hash && this.feed.tree.get(2 * i)) || !reserved.set(i, true)) {
    ...
    if (range.hash) {
      ...
      for (let j = range.start; j < wantedEnd; j++) {
...
function createView (page) {
  const buf = page ? page.buffer : EMPTY
  return new DataView(buf.buffer, buf.byteOffset, 1024)
}

function remoteAndNotLocal (local, buf, le, start) {
  const remote = new DataView(buf.buffer, buf.byteOffset)
  const len = Math.floor(buf.length / 4)
  const arr = new Uint32Array(buf.buffer, buf.byteOffset, len)
  let p = start / 8192 // 8192 is bits per bitfield page
  let l = 0
  let page = createView(local.pages.get(p++, true))
...
function nextRandom (ite, start, end) {
  const len = end - start
  const i = ite.seek(Math.floor(Math.random() * len) + start).next(true)
