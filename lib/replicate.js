...
function replicate (feed, opts) {
  var stream = (opts && opts.stream) || protocol(opts)
...
Peer.prototype.end = function () {
  if (!this._closed) {
    ...
    this.stream.close()
    if (!this.stream.stream.feeds.length && !this.live) this.stream.stream.finalize()
  }
}
...
Peer.prototype._downloadRange = function (range) {
  ...
  if (i === -1) {
    if (!range.bytes && ite.seek(range.start).next() === -1 && (range.end > -1 && this.remoteLength >= range.end)) {
