...
Peer.prototype.ondata = function (data) {
  ...
  if (!allowPush && !this.feed._reserved.get(data.index)) {
    // TODO: send back unhave message for this index, to let the remote know we rejected it
    // otherwise there is a small risk of a race condition, but worst case a timeout will
    // happen and they'll reconnect, so can be fixed later for now.
