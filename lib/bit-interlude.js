  ...
  flush (writer) {
    for (const { start, end } of this.ranges) {
      ...
      while (index < end) {
        ...
        while (index < stop) {
          const word = (index >> 5) - (page.index * (buf.byteLength >> 2))
          const byte = word << 2
          const bit = index & 31

          let mask = 0xffffffff >> bit

          if (this.drop) mask ^= 0xffffffff // flip all bits

          const current = view.getUint32(byte)
          const value = this.drop ? current & mask : current | mask
