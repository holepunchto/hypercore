...
const BYTES_PER_PAGE = 4096 // constant
...
module.exports = class BitInterlude {
  ...
  flush (writer, debug) {
    if (!this.ranges.length) return { ranges: [] }
    ...
    while (index < final) {
      const page = this.bitfield.getPage(index, false)
      const buf = b4a.allocUnsafe(page ? page.bitfield.byteLength : BYTES_PER_PAGE)
      ...
      if (page || hasValue) writer.putBitfieldPage(index, buf)
    }
