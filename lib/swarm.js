...
Swarm.prototype._use = function (Extension) {
  var name = Extension.prototype.name
  if (!name) {
    throw new Error('Extension class requires a "name" property on the prototype')
  }
  if (this.core[name]) return

  var handler = new Extension(this)
  function noop () {}
  if (typeof handler.handshake !== 'function') handler.handshake = noop

  this.extensions.push(name)
  this.core[name] = handler
}

Swarm.prototype._getid = function (name) {
  var index = this.extensions.indexOf(name)
  var id = EXTENSION_OFFSET + index
  return id
}
...
Swarm.prototype.createStream = function () {
  ...
  function handshake () {
    var available = peer.listAvailable()
    available.forEach(function (extension) {
      var id = self._getid(extension)
      var handler = self.core[extension]
      handler.handshake(id, peer)
    })
  }
}
