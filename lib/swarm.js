...
Swarm.prototype._get = function (link) {
  ...
  var subswarm = {
    ...
    fetch: fetch,
    synced: false,
    requesting: bitfield(1, BITFIELD_OPTIONS),
    open: open
  }
  ...
  function open (feed) {
    ...
    subswarm.feed.open(syncingMaybe)
    ...
    function syncingMaybe () {
      if (!feed.blocks) return
      for (; ptr < feed.blocks; ptr++) {
        if (!feed.has(ptr)) return
      }
      subswarm.synced = true
    }
  }

  function fetch (peer) {
    if (!subswarm.feed || !subswarm.feed.opened || subswarm.synced) return
  ...
  function fetchPeer (peer) {
    ...
    if (peer.remotePausing || subswarm.synced) return
    while (true) {
      ...
      subswarm.requesting.set(block, true)
      peer.request(block)
  ...
  function chooseBlock (peer) {
    ...
    for (var j = 0; j < subswarm.feed.want.length; j++) {
      ...
      if (subswarm.requesting.get(block)) continue
    ...
    for (var i = 0; i < len; i++) {
      block = (offset + i) % len
      if (peer.amRequesting.get(block) || subswarm.requesting.get(block)) continue
      if (peer.remoteBitfield.get(block) && !subswarm.feed.bitfield.get(block)) {
...
Swarm.prototype.createStream = function () {
  ...
  function onchannel (ch) {
    ...
    ch.on('leave', function () {
      var len = ch.amRequesting.buffer.length * 8
      for (var i = 0; i < len; i++) {
        if (ch.amRequesting.get(i)) {
          subswarm.requesting.set(i, false)
        }
      }
      subswarm.fetch()
    })
    ...
    ch.on('request', function (block) {
      ...
      subswarm.feed.get(block, function (err, data) {
        ...
        subswarm.feed.proof(block, function (err, proof) {
          if (err) return ch.leave(err)
          subswarm.feed.emit('get', block, data)
