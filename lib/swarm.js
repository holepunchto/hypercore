...
Swarm.prototype._use = function (name, cb) {
  if (!cb) cb = function () {}
  var index = this.extensions.indexOf(name)
  if (index > -1) return EXTENSION_OFFSET + index
  this.extensions.push(name)
  var id = EXTENSION_OFFSET + (this.extensions.length - 1)
  if (!this.callbacks[id]) this.callbacks[id] = cb
  return id
}
...
Swarm.prototype.createStream = function () {
  ...
  function handshake () {
    var exts = Object.keys(self.callbacks)
    for (var i = 0; i < exts.length; i++) {
      var id = exts[i]
      if (!peer.supports(id)) continue
      self.callbacks[id](id, peer)
    }
  }
}
