...
Feed.prototype.get = function (block, cb) { // TODO: on static feeds return null if > blocks
  ...
  if (this.storage) this.storage.get(block, done)
...
// verify and put a single block of data
// "proof" will contain, at least, the leaf node representing the block in the merkle tree
// it may also contain its sibling leaf, as well as ancestor nodes
Feed.prototype.put = function (block, data, proof, cb) {
  if (!this.opened) return this._openAndPut(block, data, proof, cb)

  // convert to the value-encoding before verification and storage
  data = this.valueEncoding.encode(data)

  var self = this
  var offset = 0 // used to track progress through the proof-nodes
  var nodes = proof.nodes || []

  // all database operations are going to be batched
  // if verification fails, nothing will be saved
  var batch = []

  // "top" is used to track the merkle-tree node which we're currently verifying.
  // it is generated, initially, from the block's data, and starts as a leaf.
  // then, it bubbles up the tree-ancestry for verification.
  // as it bubbles, it updates its index, size, and hash -- with the hash becoming
  // a hash of itself and of its siblings on each step.
  var top = {
    index: 2 * block,
    size: data.length,
    hash: hash.data(data)
  ...
  function loop (err, node) {
    ...
    // have we read the equivalent node for current 'top' from disk?
    // if so, then we've reached already-confirmed nodes in the tree,
    // and this will be our final verification.
    ...
    // bubble up the merkle tree until either
    //  A) "top" contains a locally-stored node we can verify against
    //  B) we've reached a sibling that's locally-stored, and needs to be loaded
    //  C) we reach a new root, which was given in the "proof" passed to this function
    ...
    // if verified == true, then we've reached a node that we have verified on disk
    // load it up and continue verifying "top" with it
    if (verified) return self._core._nodes.get(self._prefix + top.index, loop)
    // similar case, we have a verified sibling to verify against
    if (self.tree.get(sibling)) return self._core._nodes.get(self._prefix + sibling, loop)

    // we've reached a new root: run additional verification, which includes a signature check
  ...
  function finalize (err) {
    ...
    // update the local tree verifications
    self.tree.set(2 * block)
    for (var i = 0; i < offset; i++) self.tree.set(nodes[i].index)

    // update the local block bitfield
    if (self.bitfield.set(block, true)) self.emit('have', block, data)

    self._first = false // first message has been written
    self._sync(null, cb) // flush to disk
    self._drain(block) // update selections and call any waiting get() cbs
  }
}
...
// helper for put() to write new root-node received from a peer
// runs signature verificaiton and updates the TODO what? before writing
Feed.prototype._putRoots = function (top, batch, nodes, nodesOffset, sig, cb) {
  var self = this

  // calculate the indexes of the roots that need to be used in verification,
  // then allocate the 'roots' array to contain them
  ...
  function loop (err, node) {
    ...
    // populate the roots array with nodes, either passed into the method,
    ...
    // verify signature if a live feed
    // verify checksum if a static feed
    if (sig) {
    ...
    // write indentifacation metadata if this is the first write
    if (self._first) {
      batch.push({
        type: 'put',
