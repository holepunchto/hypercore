...
Feed.prototype.put = function (block, data, proof, cb) {
  ...
  batch = batch.concat(self._nodeBatch(top))
  loop(null, null)
...
Feed.prototype._putRoots = function (top, batch, nodes, nodesOffset, sig, cb) {
  ...
  function loop (err, node) {
    ...
    for (var i = 0; i < roots.length; i++) {
      ...
      batch = batch.concat(self._nodeBatch(roots[i]))
    }
    ...
    if (indexes.length) {
      ...
      if (newBlocks > self.blocks) {
        ...
        batch.push({
          type: 'put',
          key: '!heads!' + self._prefix + checksum.toString('hex'),
          value: String(roots.length ? roots[0].index : 0)
        })
        self.emit('update')
      }
    }
...
Feed.prototype._append = function (buffers, cb) {
  ...
  for (var i = 0; i < buffers.length; i++) {
    ...
    for (var j = 0; j < nodes.length; j++) {
      ...
      batch = batch.concat(this._nodeBatch(node))
    }
  }
...
Feed.prototype.getBlockFromHash = function (hash, cb) {
  var self = this
  if (self.opened) onopen()
  else self.open(onopen)

  function onopen (err) {
    if (err) return cb(err)
    var key = '!heads!' + self._prefix + hash.toString('hex')
    self._db.get(key, function (err, block) {
      if (err) cb(err)
...
Feed.prototype._nodeBatch = function (node) {
  var h = hash.tree(this._merkle.roots)
  return [
    {
      type: 'put',
    ...
    {
      type: 'put',
      key: '!heads!' + this._prefix + h.toString('hex'),
      value: String(this.blocks)
    }
  ]
}
