...
Feed.prototype._seek = function (bytes, cb) {
  if (!bytes) return cb(null, 0, 0)

  var roots = flat.fullRoots(2 * this.blocks)
  if (!roots.length) return cb(new Error('Can not seek to byte offset'))

  var self = this
  var offset = 0

  this._core._nodes.get(this._prefix + roots[offset++], onroot)

  function onroot (err, node) {
    if (err) return cb(err)
    if (node.size > bytes) return onparent(null, node)

    if (offset >= roots.length) return cb(new Error('Can not seek to byte offset'))
    bytes -= node.size
    self._core._nodes.get(self._prefix + roots[offset++], onroot)
  }

  function onparent (err, node) {
    if (err) return cb(err)
    if (!(node.index & 1)) return cb(null, node.index / 2, bytes)
    self._core._nodes.get(self._prefix + flat.leftChild(node.index), onleft)
  }

  function onleft (err, node) {
    if (err) return cb(err)
    if (node.size > bytes) return onparent(null, node)

    bytes -= node.size
    self._core._nodes.get(self._prefix + flat.sibling(node.index), onparent)
  }
}
