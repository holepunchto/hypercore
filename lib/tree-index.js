...
TreeIndex.prototype.proof = function (index, opts) {
  ...
  const nodes = []
  const remoteTree = opts.tree || new TreeIndex()
  let digest = opts.digest || 0
  ...
  while (!remoteTree.get(next)) {
    ...
    if (!this.get(sibling)) {
      ...
      const verifiedBy = this.verifiedBy(next)
...
TreeIndex.prototype.digest = function (index) {
  ...
  let digest = 0
  let next = flat.sibling(index)
  const max = Math.max(next + 2, this.bitfield.length) // TODO: make this less ... hacky

  let bit = 2
  let depth = flat.depth(index)
  let parent = flat.parent(next, depth++)
...
TreeIndex.prototype.blocks = function () {
  let top = 0
  let next = 0
  const max = this.bitfield.length
...
TreeIndex.prototype.verifiedBy = function (index, nodes) {
  ...
  let depth = flat.depth(index)
  let top = index
  let parent = flat.parent(top, depth++)
...
function addFullRoots (verifiedBy, nodes, root, remoteTree) {
  const roots = flat.fullRoots(verifiedBy)
  for (let i = 0; i < roots.length; i++) {
