...
module.exports = class RemoteBitfield {
  ...
  set(index, val) {
    ...
    if (!p && val) {
      ...
      if (this._maxPages <= i) this._maxPages = i + 1
  ...
  setRange(start, end, val) {
    ...
    // Cap end if val = false
    if (!val) end = Math.min(end, this._maxPages * BITS_PER_PAGE)
  ...
  findLast(val, position) {
    ...
    if (i >= this._maxSegments) return -1
  ...
  clear(start, bitfield) {
    ...
    while (length > 0) {
      ...
      if (p) p.clear(j, bitfield.subarray(0, range / 32))
