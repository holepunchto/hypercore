...
module.exports = async function auditCore (core, { tree = true, blocks = true, bitfield = true, dryRun = false } = {}) {
  ...
  const stats = { treeNodes: 0, blocks: 0, bits: 0, droppedRootNodes: 0, droppedTreeNodes: 0, droppedBlocks: 0, droppedBits: 0 }
  ...
  if (tree) {
    ...
    while (stack.length > 0) {
      ...
      if (isBadTree(node, leftNode, rightNode)) {
        if (!tx && !corrupt) tx = core.state.storage.write()
...
function clear (core) {
  const tx = core.storage.write()
  tx.deleteBitfieldRange(0, -1)
