...
function computeChecksum (buf) {
  return twosComplement(crc32(buf))
}
...
const blockUpdate = { // TODO: can be folded into a HAVE later on with the most recent spec
  preencode (state, b) {
    state.end++ // flags
    c.uint.preencode(state, b.index)
...
const blockUpdateArray = c.array(blockUpdate)

exports.oplogEntry = {
  preencode (state, m) {
    state.end++ // flags
    if (m.userData) keyValueArray.preencode(state, m.userData)
    if (m.treeNodes) nodeArray.preencode(state, m.treeNodes)
    if (m.treeUpgrade) treeUpgrade.preencode(state, m.treeUpgrade)
    if (m.blocks) blockUpdateArray.preencode(state, m.blocks)
  },
  encode (state, m) {
    state.buffer[state.start++] = (m.userData ? 1 : 0) | (m.treeNodes ? 2 : 0) | (m.treeUpgrade ? 4 : 0) | (m.blocks ? 8 : 0)
    if (m.userData) keyValueArray.encode(state, m.userData)
    if (m.treeNodes) nodeArray.encode(state, m.treeNodes)
    if (m.treeUpgrade) treeUpgrade.encode(state, m.treeUpgrade)
    if (m.blocks) blockUpdateArray.encode(state, m.blocks)
  },
  decode (state) {
    const flags = c.uint.decode(state)
    return {
      userData: (flags & 1) !== 0 ? keyValueArray.decode(state) : null,
      treeNodes: (flags & 2) !== 0 ? nodeArray.decode(state) : null,
      treeUpgrade: (flags & 4) !== 0 ? treeUpgrade.decode(state) : null,
      blocks: (flags & 8) !== 0 ? blockUpdateArray.decode(state) : null
    }
  }
}
...
const blocksHeader = {
  preencode (state, b) {
    c.string.preencode(state, b.bitfieldType)
    c.uint.preencode(state, b.length)
    c.uint.preencode(state, b.byteLength)
  },
  encode (state, b) {
    c.string.encode(state, b.bitfieldType)
    c.uint.encode(state, b.length)
    c.uint.encode(state, b.byteLength)
  },
  decode (state) {
    return {
      bitfieldType: c.string.decode(state),
      length: c.uint.decode(state),
...
exports.oplogHeader = {
  preencode (state, h) {
    ...
    blocksHeader.preencode(state, h.blocks)
  },
  encode (state, h) {
    ...
    blocksHeader.encode(state, h.blocks)
  },
  decode (state) {
    ...
    return {
      ...
      blocks: blocksHeader.decode(state)
    }
  }
}
