...
const infoSignerArray = c.array(infoSigner)

const multipleSigners = {
  preencode (state, m) {
    state.end++ // flags small uint
    c.uint.preencode(state, m.quorum)
    infoSignerArray.preencode(state, m.signers)
  },
  encode (state, m) {
    c.uint.encode(state, m.allowPatched ? 1 : 0)
    c.uint.encode(state, m.quorum)
    infoSignerArray.encode(state, m.signers)
  },
  decode (state) {
    ...
    return {
      ...
      signers: infoSignerArray.decode(state)
    }
  }
}
...
const manifest = exports.manifest = {
  preencode (state, m) {
    ...
    if (m.signer) {
      infoSigner.preencode(state, m.signer)
    }
  ...
  encode (state, m) {
    ...
    if (m.signer) {
      infoSigner.encode(state, m.signer)
    }
  ...
  decode (state) {
    ...
    return {
      ...
      signer: type === 1 ? infoSigner.decode(state) : null,
...
wire.request = {
  ...
  encode (state, m) {
    // TODO: when we major bump the protocol swap the manifest / priority flag
    const flags = (m.block ? 1 : 0) | (m.hash ? 2 : 0) | (m.seek ? 4 : 0) | (m.upgrade ? 8 : 0) | (m.manifest ? 32 : 0) | (m.priority ? 16 : 0)
...
oplog.header = {
  ...
  encode (state, h) {
    ...
    c.uint.encode(state, (h.manifest ? 1 : 0) | (h.keyPair ? 2 : 0))
  ...
  decode (state) {
    ...
    if (version === 0) {
      ...
      return {
        ...
        tree: old.tree,
        hints: old.hints,
        contiguousLength: old.contiguousLength
      }
    }
    ...
    return {
      ...
      manifest: (flags & 1) !== 0 ? manifest.decode(state) : null,
      keyPair: (flags & 2) !== 0 ? keyPair.decode(state) : null,
      userData: keyValueArray.decode(state),
      tree: treeHeader.decode(state),
      hints: hints.decode(state),
...
const proofsEncoding = c.array({
  ...
  decode (state) {
    return {
      ...
      patch: c.uint.decode(state) ? patchEncoding.decode(state) : null
    }
  }
})

const nodesEncoding = c.array({
  preencode (state, n) {
    c.uint.preencode(state, n.index)
    c.uint.preencode(state, n.size)
    c.fixed32.preencode(state, n.hash)
...
const multisignature = exports.multisignature = {
  preencode (state, s) {
    c.uint.preencode(state, s.length)
    proofsEncoding.preencode(state, s.proofs)
    nodesEncoding.preencode(state, s.nodes)
  },
  encode (state, s) {
    c.uint.encode(state, s.length)
    proofsEncoding.encode(state, s.proofs)
    nodesEncoding.encode(state, s.nodes)
  },
  decode (state) {
    return {
      length: c.uint.decode(state),
      proofs: proofsEncoding.decode(state),
      nodes: nodesEncoding.decode(state)
    }
  }
}