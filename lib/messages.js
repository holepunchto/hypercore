...
const mutation = {
  preencode (state, m) {
    ...
    uintArray.preencode(state, m.blocks)
  ...
  encode (state, m) {
    ...
    uintArray.encode(state, m.blocks)
    c.buffer.encode(state, m.signature)
  },
  decode (state) {
    return {
      type: c.uint.encode(state),
      fork: c.uint.encode(state),
      length: c.uint.encode(state),
      nodes: nodeArray.encode(state),
      blocks: uintArray.encode(state),
      signature: c.buffer.encode(state)
    }
  }
}
