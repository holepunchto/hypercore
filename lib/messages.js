...
const infoSigner = {
  preencode (state, m) {
    ...
    c.fixed32.preencode(state, m.entry)
...
const dataUpgrade = {
  preencode (state, u) {
    ...
    if (u.start === 0 && u.manifest !== null) manifest.preencode(state, u.manifest)
  },
  encode (state, u) {
    c.uint.encode(state, u.start)
    c.uint.encode(state, u.length)
    ...
    if (u.start === 0 && u.manifest !== null) manifest.encode(state, u.manifest)
  },
  decode (state) {
    ...
    return {
      ...
      manifest: start === 0 && state.start < state.end ? manifest.decode(state) : null
    }
  }
}
