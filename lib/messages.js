...
const mutation = {
  preencode (state, m) {
    ...
    nodeArray.preencode(state, m.blocks)
    exports.data.preencode(state, m.proof)
  },
  encode (state, m) {
    c.uint.encode(state, m.length)
    c.uint.encode(state, m.byteLength)
    c.uint.encode(state, m.fork)
    nodeArray.encode(state, m.blocks)
    exports.data.encode(state, m.proof)
  },
  decode (state) {
    return {
      length: c.uint.decode(state),
      ...
      fork: c.uint.decode(state),
      blocks: nodeArray.decode(state),
      proof: exports.data.decode(state)
    }
  }
}
...
exports.oplog.op = exports.checksumEncoding({
  preencode (state, op) {
    ...
    if (op.mutation) mutation.preencode(state, op.local)
