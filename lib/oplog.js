...
module.exports = class Oplog extends Flushable {
  ...
  async _flush () {
    ...
    for (const op of queue) {
      op.batch.commit()
      if (op.batch.signature) latestSignature = op.batch.signature
      if (op.values) {
        for (let i = 0; i < op.values.length; i++) {
          console.log('setting bitfield:', op.index + i)
          this.core.bitfield.set(op.index + i, true)
        }
      }
    }
    for (let i = this.core.tree.length; i < oldLength; i++) {
  ...
  async put (batch, index, value) {
    ...
    if (!batch.commitable()) return false
    console.log('pushing into queue')
    this._queue.push(new Op(Op.PUT, batch, index, value ? [value] : null))
    console.log('flushing, this.flush:')
    await this.flush()
