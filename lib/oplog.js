...
module.exports = class Oplog {
  ...
  async append (op) {
    ...
    try {
      ...
      const encoded = c.encode(oplogMessages.op, op)
      await write(this.storage, METADATA_LENGTH + this._logLength, op, oplogMessages.op)
      this._logLength += encoded.length
    } finally {
      this._appending = false
    }
  }
...
async function write (storage, offset, buf, encoding) {
  const encoded = encoding ? c.encode(encoding, buf) : buf
  return new Promise((resolve, reject) => {
    storage.write(offset, encoded, err => {
      if (err) return reject(err)
      return resolve()
    })
  })
