...
const DEFAULT_ENTROPY = b4a.alloc(32, 0)

module.exports = createAuth

function createAuth (manifest, opts) {
  const namespace = manifest.namespace || DEFAULT_NAMESPACE
  ...
  throw new Error('Invalid manifest: no signer was provided')
}
...
function singleSigner (signer, ns, opts = {}) {
  ...
  const namespacer = opts.compat ? compatNamespaced : namespaced

  const sign = opts.sign
    ? opts.sign
    : keyPair.secretKey
      ? (signable) => crypto.sign(namespacer(signable, ns, entropy), keyPair.secretKey)
      : undefined

  return {
    sign,
    verify (signable, signature) {
      return crypto.verify(namespacer(signable, ns, entropy), signature, keyPair.publicKey)
    }
  }
}
...
function compatNamespaced (signable) {
  return signable
}

function namespaced (signable, namespace, entropy) {
  return b4a.concat([namespace, entropy, signable])
}
