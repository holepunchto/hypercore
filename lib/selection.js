...
Selection.prototype.next = function (remoteBitfield) {
  for (var i = this._ranges.length - 1; i >= 0; i--) {
    ...
    for (var j = 0; j < list.length; j++) {
      ...
      var blk = this._next(range, remoteBitfield)
      if (blk > -1) {
...
Selection.prototype._next = function (range, remoteBitfield) {
  switch (range.strategy) {
    case 0: return this._nextRandom(range, remoteBitfield)
    case 1: return this._nextLinear(range, remoteBitfield)
  }
}

Selection.prototype._nextLinear = function (range, remoteBitfield) {
  var end = Math.min(range.end, remoteBitfield.length)
...
Selection.prototype._nextRandom = function (range, remoteBitfield) {
  var end = Math.min(range.end, remoteBitfield.length)
