...
module.exports = class Core {
  ...
  async copyPrologue (src, { additional = [] } = {}) {
    ...
    try {
      ...
      logs.push('get srcLength ' + srcLength + ' ' + src.tree.length)
      ...
      while (segmentEnd < srcLength) {
        ...
        logs.push('segment ' + segmentStart + ' ' + segmentEnd + ' - ' + srcLength + ' ' + minimumSegmentEnd(segmentStart, src.bitfield, this.bitfield))
        logs.push('getNeededNodes ' + srcLength + ' ' + segmentStart + ' ' + segmentEnd)
      ...
      console.log('hadPending', this.wasPending, src.wasPending)
      console.log('first five bits:', src.bitfield.get(0), src.bitfield.get(1), src.bitfield.get(2), src.bitfield.get(3), src.bitfield.get(4))
  ...
  async insertBatch (batch, values, { signature, keyPair = this.header.keyPair, pending = false, treeLength = batch.treeLength } = {}) {
    ...
    if (pending) this.wasPending = true
