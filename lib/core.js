...
module.exports = class Core {
  ...
  static async resume (storage, opts) {
    ...
    let header = await loadHeader(storage)
    ...
    if (!header || overwrite) {
      ...
      await flushHeader(storage, header)
    }
  ...
  async audit () {
    ...
    try {
      ...
      if (corrections.blocks || corrections.tree) await this._flushBitfield()
      return corrections
    } finally {
  ...
  _setManifest (writer, manifest, keyPair) {
    ...
    writer.setCoreAuth({ key: this.header.key, manifest })

    this.compat = verifier.compat
    this.verifier = verifier
    this._manifestFlushed = false

    this._updates.push({ status: 0b10000, bitfield: null, value: null, from: null })
  }
  ...
  async _writeBlock (writer, index, value) {
    await this.blocks.put(writer, index, value)
  }

  userData (key, value) {
    const w = this.storage.createWriteBatch()
    w.setUserData(key, value)
    return w.flush()
  }
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      const offset = await this.tree.getByteOffset(start * 2)
      const endOffset = await this.tree.getByteOffset(end * 2)

      const length = endOffset - offset

      await this.blocks.clear(writer, start, length)
      await writer.flush()
  ...
  async append (values, { signature, keyPair = this.header.keyPair, preappend } = {}) {
    ...
    try {
      ...
      const byteLength = await this._appendBlocks(writer, values)
  ...
  async _verifyExclusive (writer, { batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (bitfield) await this._writeBlock(writer, bitfield.start, value)
      ...
      this._updates.push({ status, bitfield, value, from })
    } finally {
      this.updating = false
      this._mutex.unlock()
    }
  ...
  async _verifyShared () {
    ...
    try {
      ...
      for (const { batch, bitfield, value } of verifies) {
        ...
        if (bitfield) {
          await this._writeBlock(writer, bitfield.start, value)
        }
      }
  ...
  async verify (proof, from) {
    ...
    this._verifies = [op]
    this._verified = executeVerify(writer, this, this._verifyShared())
  ...
  async _truncate (writer, batch, from) {
    ...
    this._updates.push({ status, bitfield, value: null, from })
  }
  ...
  async _flushUpdates () {
    ...
    this._updates = []
    for (const { status, bitfield, value, from } of updates) {
...
async function loadHeader (storage) {
  const read = await storage.createReadBatch()

  const auth = read.getCoreAuth()
  const keyPair = read.getLocalKeyPair()
  const head = read.getCoreHead()

  await read.flush()

  if (!(await head)) return null

  return {
    key: (await auth).key,
    manifest: Verifier.decodeManifest((await auth).manifest),
    external: null,
...
async function flushHeader (storage, header) {
  const write = await storage.createWriteBatch()

  const { key, manifest, keyPair } = header

  write.setCoreAuth({ key, manifest: Verifier.encodeManifest(manifest) })
  write.setLocalKeyPair(keyPair)
