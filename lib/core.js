  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.tree.length, treeLength = state.treeLength } = {}) {
    ...
    try {
      ...
      if (this.verifier === null) {
        throw INVALID_OPERATION() // easier to assert than upsert
      } else if (signature && !this.verifier.verify(state.tree.batch(), signature)) {
        throw INVALID_SIGNATURE() // invlaid sig
      ...
      const from = treeLength * 2
      const to = length * 2

      for (const ite = flat.iterator(from); flat.rightSpan(ite.index) < to; ite.parent()) {
        if (ite.index < from) treePromises.push(reader.getTreeNode(ite.index))
      }
      ...
      for (let i = treeLength; i < length; i++) {
        ...
        treePromises.push(reader.getTreeNode(treeIndex))
        treePromises.push(reader.getTreeNode(treeIndex + 1))
      ...
      const lastPage = getBitfieldPage(length)

      const index = treeLength

      for (let i = firstPage; i <= lastPage; i++) {
        const pageBuffer = b4a.alloc(Bitfield.BYTES_PER_PAGE) // TODO: move to bitfield

        const start = getBitfieldOffset(index)

        const last = ((i + 1) * Bitfield.BITS_PER_PAGE) - 1
        const end = getBitfieldOffset(length > last ? last : length)

        quickbit.fill(pageBuffer, start, end, true)

        writer.putBitfieldPage(i, pageBuffer)
      }

      const tree = {
        fork: this.tree.fork,
        length,
        rootHash: this.crypto.tree(roots),
        signature
      }

      const upgraded = this.tree.length < length
      ...
      this.bitfield.setRange(treeLength, length - treeLength, true)

      const bitfield = { start: treeLength, length: length - treeLength, drop: false }
      const status = upgraded ? 0b0001 : 0

      this.onupdate({ status, bitfield, value: null, from: null })

      state.treeLength = this.tree.length
