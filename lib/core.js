...
class SessionState {
  ...
  get treeLength () {
    return this.isDefault ? this.length : this.storage.dependencies[this.storage.dependencies.length - 1].length
  }
...
module.exports = class Core {
  ...
  async createSession (name, length, overwrite) {
    ...
    return new SessionState(this, storage, this.blocks, tree, bitfield, treeInfo.length)
  }
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.tree.length, treeLength = state.treeLength } = {}) {
    ...
    try {
      ...
      const deps = state.storage.dependencies
      const dependency = deps[deps.length - 1]

      dependency.length = batch.length

      srcUpdate.batch.setDataDependency(dependency)
