...
const DEPENDENCY = Symbol.for('dependency')
...
class Update {
  ...
  updateDependency (length) {
    const dependencies = this.state.storage.dependencies
    if (!dependencies.length) return // skip default state and overlays of default
    ...
    const dependency = { data: target.data, length }

    this.batch.setDataDependency(dependency)
    this.updates.push({ type: DEPENDENCY, update: dependency })
  }

  updateContig (bitfield) {
...
class SessionState {
    ...
    for (const { type, update } of u.updates) {
      switch (type) {
        ...
        case DEPENDENCY: // dependency
          this._updateDependencies(update)
          break
      }
    }
  ...
  async truncate (length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      if (batch.length < this.flushedLength()) update.updateDependency(batch.length)

      await this.flushUpdate(update)

      this.ontruncate(length, batch.treeLength, fork)
    } finally {
      this._unlock()
    }
  }
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      update.updateContig(bitfield)

      if (start < this.flushedLength()) update.updateDependency(start)
  ...
  onappend (bitfield) {
    if (this.isDefault()) {
      if (bitfield) {
        this.core.replicator.cork()
        this.core.replicator.onupgrade()
        this.core.replicator.onhave(bitfield.start, bitfield.length, bitfield.drop)
        this.core.replicator.uncork()
      } else {
        this.core.replicator.onupgrade()
      }
    }
  ...
  ontruncate (to, from, fork) {
    if (this.isDefault()) {
      const length = from - to

      this.core.replicator.cork()
      this.core.replicator.ontruncate(to, length)
      this.core.replicator.onhave(to, length, true)
      this.core.replicator.onupgrade()
      this.core.replicator.uncork()
    ...
    for (let i = this.sessions.length - 1; i >= 0; i--) {
      this.sessions[i].emit('truncate', to, fork)
    }
  }
...
module.exports = class Core {
  ...
  checkIfIdle () {
    if (this.hasSession() === false && this.replicator.idle() && this.state !== null && this.state.mutex.idle()) this.onidle()
  }
  ...
  async _tryOpen (opts) {
    ...
    this.state = new SessionState(this, storage, this.blocks, tree, bitfield, -1, null)
  ...
  async createSession (name, length, overwrite) {
    ...
    const bitfield = await Bitfield.open(storage, treeInfo.length)
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.tree.length, treeLength = state.flushedLength(), overwrite = false } = {}) {
    ...
    try {
      ...
      if (treeLength < length) {
        ...
        source.updateDependency(length)
      ...
      this.bitfield.setRange(treeLength, length, true)
  ...
  async _verifyExclusive ({ batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (batch.upgraded) this.state.onappend(bitfield)
      else if (bitfield) this.replicator.onhave(bitfield.start, bitfield.length, bitfield.drop)
    } finally {
  ...
  async _verifyShared () {
    ...
    try {
      ...
      for (let i = 0; i < verifies.length; i++) {
        ...
        if (bitfield) update.updateContig(bitfield)
      ...
      for (let i = 0; i < verifies.length; i++) {
        ...
        if (bitfield) this.replicator.onhave(bitfield.start, bitfield.length, bitfield.drop)
      }
    } finally {
      this.state._clearActiveBatch()
      this.state.mutex.unlock()
  ...
  async reorg (batch, from) {
    ...
    try {
      ...
      this.state.ontruncate(batch.ancestors, batch.treeLength, batch.fork)
    } finally {
      this.state._clearActiveBatch()
      this.truncating--
      this.state.mutex.unlock()
