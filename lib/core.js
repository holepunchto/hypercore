...
module.exports = class Core {
  ...
  static async open (storage, rocksdb, opts = {}) {
    ...
    try {
      return await this.resume(rocksdb, oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts)
    } catch (err) {
  ...
  async truncate (length, fork, { signature, keyPair = this.header.keyPair } = {}) {
    ...
    try {
      ...
      const truncating = this._truncate(writer, batch, null)
      writer.flush()
  ...
  async _verifyExclusive (writer, { batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (bitfield) this._writeBlock(bitfield.start, value)
  ...
  async verify (proof, from) {
    ...
    if (batch.upgraded) {
      const verifying = this._verifyExclusive(writer, op)
      writer.flush()
  ...
  async reorg (batch, from) {
    ...
    try {
      if (!batch.commitable()) return false
      const writer = this.storage.createWriteBatch()
      const truncating = this._truncate(writer, batch, from)
      writer.flush()
