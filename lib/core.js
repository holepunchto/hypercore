...
module.exports = class Core {
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts) {
    ...
    const prologue = (header.manifest && header.manifest.multipleSigners) ? header.manifest.multipleSigners.prologue : null
  ...
  async copyFrom (src, signature, { length = src.tree.length, additional = [] } = {}) {
    ...
    try {
      ...
      if (src.tree.length > this.tree.length) {
        this.tree.fork = src.tree.fork

        const batch = length < src.tree.length
          ? await src.tree.truncate(length)
          : src.tree.batch()
      ...
      if (length > src.tree.length) {
        ...
        await this.blocks.putBatch(src.tree.length, blocks, src.tree.byteLength)
        this.bitfield.setRange(src.tree.length, missing, true)

        updates.push({
          drop: false,
          start: src.tree.length,
          length: missing
        })

        const upgrade = src.tree.length > this.tree.length
        const batch = upgrade ? this.tree.batch() : await this.tree.truncate(src.tree.length)
      ...
      // verify if upgraded
      if (this.tree.length > initialLength) {
        try {
