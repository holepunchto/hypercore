...
const CORE_BATCH_ID = 0

module.exports = class Core {
  constructor (storage, header, compat, crypto, oplog, bigHeader, tree, blocks, bitfield, verifier, sessions, legacy, onupdate, onconflict) {
  ...
  // todo: storage should obv only be rocksdb, hack for now
  static async open (storage, opts = {}) {
    const db = await storage('db')
    const oplogFile = await storage('oplog')
    const treeFile = db
    const bitfieldFile = await storage('bitfield')
    const dataFile = db
    const headerFile = await storage('header')

    try {
      return await this.resume(db, oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts)
    } catch (err) {
  ...
  static async resume (storage, oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts) {
    ...
    const tree = await MerkleTree.open(treeFile, CORE_BATCH_ID, { crypto, prologue, ...header.tree })
    ...
    if (overwrite) {
      tree.clear(writer, CORE_BATCH_ID)
      blocks.clear(writer, CORE_BATCH_ID)
    ...
    return new this(storage, header, compat, crypto, oplog, bigHeader, tree, blocks, bitfield, verifier, opts.sessions || [], legacy, opts.onupdate || noop, opts.onconflict || noop)
  }
  ...
  _appendBlocks (writer, id, values) {
    return this.blocks.putBatch(writer, id, this.tree.length, values)
  }

  async _writeBlock (writer, id, index, value) {
    await this.blocks.put(writer, id, index, value)
  }
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      this.blocks.clear(writer, offset, length)
      await writer.flush()
  ...
  async append (values, { signature, keyPair = this.header.keyPair, preappend } = {}) {
    ...
    try {
      ...
      this._appendBlocks(writer, CORE_BATCH_ID, values)
      ...
      const byteLength = getByteLength(values)
  ...
  async _verifyShared (writer) {
    ...
    try {
      ...
      for (let i = 0; i < verifies.length; i++) {
        ...
        if (bitfield) {
          this._setBitfield(bitfield.start, true)
          verifies[i].status = updateContig(this.header, bitfield, this.bitfield)
        }
      ...
      for (const { status, bitfield, value, from } of verifies) {
        this.onupdate(status, bitfield, value, from)
      }
  ...
  async verify (proof, from) {
    ...
    if (batch.upgraded) {
      return this._verifyExclusive(writer, op)
    }
    ...
    this._verifies = [op]
    this._verified = this._verifyShared(writer)
