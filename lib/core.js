...
class Update {
  ...
  updateDependency (length) {
    ...
    const deps = this.state.storage.dependencies
    const dependency = deps[deps.length - 1]

    dependency.length = length
...
class SessionState {
  ...
  get treeLength () {
    ...
    return this.storage.dependencies[this.storage.dependencies.length - 1].length
  }
  ...
  async truncate (length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      if (batch.length < this.treeLength) update.updateDependency(batch.length)
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      if (start < this.treeLength) update.updateDependency(start)
