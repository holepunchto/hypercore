...
const { manifestHash, createVerifier, createManifest, defaultSignerManifest } = require('./manifest')
...
module.exports = class Core {
  constructor (header, crypto, oplog, bigHeader, tree, blocks, bitfield, verifier, legacy, onupdate, onconflict) {
    ...
    this.preupdate = null
    this.header = header
    this.compat = isCompat(header.key, header.manifest)
    this.crypto = crypto
    this.oplog = oplog
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts) {
    ...
    const compat = opts.compat === true || (opts.compat !== false && !opts.manifest)
    ...
    if (!header || overwrite) {
      ...
      const noManifest = !opts.manifest && (compat || !!keyPair)
      const manifest = noManifest ? defaultSignerManifest(opts.key || keyPair.publicKey) : createManifest(opts.manifest)
    ...
    if (opts.manifest) {
      if (compat && !b4a.equals(header.key, opts.manifest.signer.publicKey)) {
        throw BAD_ARGUMENT('Key must match publicKey when in compat mode')
      }

      // if we provide a manifest and no key, verify that the stored key is the same
      if (!opts.key && !b4a.equals(header.key, manifestHash(createManifest(opts.manifest)))) {
    ...
    const verifier = header.manifest ? createVerifier(header.manifest, { compat, crypto }) : null
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    const verifier = this.verifier || createVerifier(manifest, { compat: isCompat(this.header.key, manifest), crypto: this.crypto })
