...
module.exports = class Core {
  ...
  _addUpdate (writer, update) {
    writer.userData.push(update)
  }
  ...
  _setManifest (writer, manifest, keyPair) {
    ...
    this._addUpdate(writer, { type: UPDATE, entry: { status: 0b10000, bitfield: null, value: null, from: null } })
  }
  ...
  async _flushBitfield (writer) {
    const entry = await this.bitfield.flush(writer)
    if (entry) writer.userData.push({ type: BITFIELD, entry })
  }

  async _flushBatch (b) {
    await b.flush()
    await this._flushUpdates(b.userData)
  }
  ...
  async _flushWriter () {
    ...
    const flushing = this._flushBatch(this._activeBatch)
  ...
  //     batch.commit(writer)
  //     this._addUpdate(writer, { type: TREE, entry: batch })
  ...
  async _verifyExclusive ({ batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      this._addUpdate(writer, { type: UPDATE, entry: { status, bitfield, value, from } })
  ...
  async _truncate (writer, batch, from) {
    ...
    this._addUpdate(writer, { type: UPDATE, entry: { status, bitfield, value: null, from } })
  }
  ...
  // TODO: *CRITICAL* bug - we can't flush global updates, needs to be per batch
  async _flushUpdates (updates) {
    ...
    for (const u of updates) {
      switch (u.type) {
        case UPDATE: { // update
          const { status, bitfield, value, from } = u.entry
        ...
        case BITFIELD: // bitfield
          this.bitfield.setFlushed(u.entry)
          break
      }
    }
  }
