...
module.exports = class Core {
  ...
  async copyPrologue (src, { additional = [] } = {}) {
    ...
    try {
      ...
      if (additional.length) {
        await this.blocks.putBatch(srcLength, additional, srcBatch.byteLength)
      ...
      if (entry.bitfield) this._setBitfieldRange(entry.bitfield.start, entry.bitfield.length, true)
      await this._flushOplog()
      ...
      while (segmentEnd < srcLength) {
        ...
        segmentEnd = Math.min(segmentStart + 65536, Math.min(srcLength, minimumSegmentEnd(segmentStart, src.bitfield, this.bitfield)))
  ...
  async copyFrom (src, signature, { length = src.tree.length, sourceLength = src.tree.length, fork = src.tree.fork, additional = [] } = {}) {
    ...
    try {
      if (this.tree.length !== 0 && !(prologue && length === prologue.length)) {
        throw INVALID_OPERATION('Core is already populated')
      }

      if (prologue && length === prologue.length) {
        const srcBatch = length < src.tree.length ? await src.tree.truncate(length) : src.tree.batch()
        while (srcBatch.length < prologue.length) {
          srcBatch.append(additional[srcBatch.length - sourceLength])
        }

        if (!b4a.equals(srcBatch.hash(), prologue.hash)) {
          throw INVALID_OPERATION('Source tree is conflicting')
        }
      }
      ...
      } else if (length > this.tree.length) {
