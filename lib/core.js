...
class Update {
  ...
  processCoreUpdate ({ bitfield, status, value, from }) {
    for (const u of this._coreUpdates) this._updates.push(u)
  }
...
module.exports = class Core {
  ...
  async checkConflict (proof, from) {
    ...
    try {
      const update = this._createWriteBatch()
      this._verifyBatchUpgrade(update, batch, proof.manifest)

      await this._flushWriter(update)
    } catch {
      return true
    }
  ...
  async _truncate (update, batch, from) {
    ...
    update.bitfield.setRange(batch.ancestors, this.tree.length, false)
