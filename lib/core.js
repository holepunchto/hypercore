  ...
  async _tryOpen(opts) {
    ...
    if (opts.group) {
      if (!header.group) header.group = await this.db.createGroup(opts.group)
        ...
        throw STORAGE_CONFLICT('Core is already register to group: ' + b4a.toString(header.group.key, 'hex'))
      }
    ...
    if (header.manifest) {
      ...
      if (!hadManifest || (header.keyPair && !hadKeyPair) || header.group) {
        const tx = storage.write()
        tx.setAuth({
          key: header.key,
