...
module.exports = class Core {
  constructor(db, opts = {}) {
    ...
    this.discoveryKey =
      opts.discoveryKey || (opts.key && crypto.discoveryKey(opts.key)) || null
    this.manifest = null
    this.opening = null
    this.closing = null
    this.exclusive = null
  ...
  async _tryOpen(opts) {
    ...
    let compat =
      opts.compat === true || (opts.compat !== false && !opts.manifest)
    ...
    if (!header || overwrite) {
      ...
      if (compat) {
        if (
          opts.key &&
          opts.keyPair &&
          !b4a.equals(opts.key, opts.keyPair.publicKey)
        ) {
          throw BAD_ARGUMENT(
            'Key must match publicKey when in compat mode',
            this.discoveryKey
          )
        }
      }
      ...
      const defaultManifest =
        ...
        (!!opts.compat ||
          !opts.key ||
          !!(keyPair && b4a.equals(opts.key, keyPair.publicKey)))
      ...
      header = {
        key:
          ...
          (compat
            ...
            : Verifier.manifestHash(manifest)),
        manifest,
        keyPair: keyPair
          ? {
    ...
    if (opts.manifest) {
      ...
      if (
        !opts.key &&
        !Verifier.isValidManifest(
          header.key,
          Verifier.createManifest(opts.manifest)
        )
      ) {
        throw STORAGE_CONFLICT(
          'Manifest does not hash to provided key',
          this.discoveryKey
        )
      }
    ...
    if (opts.key && !b4a.equals(header.key, opts.key)) {
      throw STORAGE_CONFLICT(
        'Another Hypercore is stored here',
        this.discoveryKey
      )
    }
    ...
    if (
      compat &&
      header.manifest &&
      !Verifier.isCompat(header.key, header.manifest)
    ) {
      compat = false
    } else if (
      !compat &&
  ...
  async _validateCommit(state, treeLength) {
    ...
    if (this.state.length > treeLength) {
      for (const root of this.state.roots) {
        ...
        if (
          batchRoot.size !== root.size ||
          !b4a.equals(batchRoot.hash, root.hash)
        ) {
          return false
        }
      }
  ...
  _verifyBatchUpgrade(batch, manifest) {
    if (!this.header.manifest) {
      ...
      if (
        ...
        throw INVALID_SIGNATURE(
          'Proof contains an invalid manifest',
          this.discoveryKey
    ...
    const verifier =
      this.verifier ||
      new Verifier(this.header.key, Verifier.createManifest(manifest), {
        legacy: this._legacy
      })
  ...
  async checkConflict(proof, from) {
    if (
      this.state.length < proof.upgrade.length ||
      proof.fork !== this.state.fork
    ) {
    ...
    const roots = await MerkleTree.getRootsFromStorage(
      this.storage,
      proof.upgrade.length
    )
    ...
    try {
      ...
      const verifyBatch = MerkleTree.verifyFullyRemote(
        this.state,
        await treeProof.settle()
      )
  ...
  updateContiguousLength(bitfield) {
    const contig = updateContigBatch(
    ...
    if (
      contig.length !== -1 &&
      contig.length !== this.header.hints.contiguousLength
    ) {
  ...
  async _onconflict(proof) {
    ...
    const err = new Error(
      'Two conflicting signatures exist for length ' + proof.upgrade.length
    )
