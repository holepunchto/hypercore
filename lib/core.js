...
module.exports = class Core {
  ...
  async purge () {
    if (!this.oplog.storage.closed) {
      throw new Error('Cannot purge while not yet closed')
    }

    const proms = [
      new Promise((resolve, reject) => {
        const cb = (err, val) => err ? reject(err) : resolve(val)
        this.oplog.storage.unlink(cb)
      }),
      new Promise((resolve, reject) => {
        const cb = (err, val) => err ? reject(err) : resolve(val)
        this.tree.storage.unlink(cb)
      }),
      new Promise((resolve, reject) => {
        const cb = (err, val) => err ? reject(err) : resolve(val)
        this.bitfield.storage.unlink(cb)
      }),
      new Promise((resolve, reject) => {
        const cb = (err, val) => err ? reject(err) : resolve(val)
        this.blocks.storage.unlink(cb)
      })
    ]
