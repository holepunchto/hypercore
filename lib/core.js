...
module.exports = class Core {
  ...
  async createSession (name, length, overwrite) {
    ...
    return new SessionState(this, storage, this.blocks, tree, -1, name)
  }
  ...
  async audit () {
    ...
    try {
      const storage = this.state.storage.createWriteBatch()
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.tree.length, treeLength = state.flushedLength(), overwrite = false } = {}) {
    ...
    try {
      ...
      if (treeLength < length) {
        ...
        if (dependency) state._updateDependencies(dependency)
      }
  ...
  updateContiguousLength (bitfield) {
    ...
    if (contig.length !== null && contig.length !== this.header.hints.contiguousLength) {
