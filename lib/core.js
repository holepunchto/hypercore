...
module.exports = class Core {
  ...
  async _tryOpen(opts) {
    ...
    const treeInfo = {
      ...
      roots: await _maybeGetRootsFromStorage(storage, header.tree.length),
      prologue
    }
  ...
  async checkConflict(proof, from) {
    ...
    const roots = await _maybeGetRootsFromStorage(this.storage, proof.upgrade.length)
...
async function _maybeGetRootsFromStorage(storage, length) {
  try {
