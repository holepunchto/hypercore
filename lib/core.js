...
module.exports = class Core {
  ...
  async truncate (length, fork, sign) {
    ...
    try {
      ...
      batch.signature = this.defaultAuth.sign(batch, sign)
  ...
  async append (values, sign, hooks = {}) {
    await this._mutex.lock()
    ...
    try {
      ...
      batch.signature = await this.defaultAuth.sign(this._legacy ? batch.signableLegacy(hash) : batch.signable(hash), batch, sign)
