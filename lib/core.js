...
const createAuth = require('./manifest')
...
module.exports = class Core {
  ...
  static createAuth (header, opts = {}) {
    ...
    return createAuth(header.manifest, { ...opts, keyPair })
  }
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, opts) {
    ...
    const auth = header.manifest ? this.createAuth(header, opts) : null
  ...
  loadAuth (manifest, { keyPair, sign } = {}) {
    if (!b4a.equals(caps.manifestHash(manifest), caps.manifestHash(this.header.manifest))) {
      throw BAD_ARGUMENT('Manifest does not match.')
    }

    this.defaultAuth = Core.createAuth({ manifest, keyPair }, { sign, compat: this.compat })
  }

  isWritable () {
    return !!(this.defaultAuth && this.defaultAuth.sign)
  }
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    const auth = this.defaultAuth || Core.createAuth({ ...this.header, manifest }, { compat })
