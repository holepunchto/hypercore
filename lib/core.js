...
module.exports = class Core {
  ...
  async quickAudit () {
    ...
    try {
      ...
      const last = this.bitfield.findLast(true, this.header.tree.length)
      const [offset, size] = await this.tree.byteRange(last * 2)
      const byteLength = offset + size
      const length = last + 1

      const data = await new Promise((resolve) => {
        this.blocks.storage.read(0, byteLength, (_, data) => resolve(data || null))
      })

      const tree = await new Promise((resolve) => {
        this.tree.storage.read(0, length * 2 * 40 - 40, (_, data) => resolve(data || null))
      })

      let i = 0
      let nextOffset = -1
      while (i < length) {
        const has = this.bitfield.get(i)

        if (!has) {
          if (i + 1 === length) break
          i = this.bitfield.findFirst(true, i + 1)
          if (i < 0) break
          nextOffset = -1
          continue
        }
        ...
        if (nextOffset === -1) {
          nextOffset = await this.tree.byteOffset(i * 2)
        }

        const state = { start: i * 80, end: i * 80 + 40, buffer: tree }
        const len = c.uint64.decode(state)
        const expectedHash = c.fixed32.decode(state)
        const blk = data.subarray(nextOffset, nextOffset + len)
        const hash = hypercoreCrypto.data(blk)

        nextOffset += blk.byteLength

        if (!b4a.equals(hash, expectedHash)) {
          const entry = {
            userData: null,
            treeNodes: null,
            treeUpgrade: null,
