...
class SessionState {
  ...
  destroy () {
    ...
    if (head !== this) {
      this.core.sessionStates[this.index] = head
  ...
  async truncate (length, fork, { signature, keyPair } = {}) {
    ...
    try {
      ...
      if (this.isDefault()) this.core.replicator.ontruncate(batch.ancestors, batch.treeLength)
      this._emitTruncate(length, fork)
    } finally {
      this._unlock()
    }
  }
  ...
  _emitTruncate (length, fork) {
    for (let i = 0; i < this.sessions.length; i++) {
      this.sessions[i].emit('truncate', length, fork)
    }
  }
...
module.exports = class Core {
  ...
  _ontruncate (start, end) {
    this.replicator.ontruncate(start, end - start)
    for (const sessionState of this.sessionStates) {
      if (end < sessionState.snapshotCompatLength) sessionState.snapshotCompatLength = end
    }
  }
  ...
  async _verifyExclusive ({ batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (batch.upgraded) this.state._emitAppend()
    } finally {
  ...
  async reorg (batch, from) {
    ...
    try {
      ...
      this._ontruncate(batch.ancestors, batch.treeLength)
      this.state._emitTruncate(batch.ancestors, batch.fork)
    } finally {
      this.state._clearActiveBatch()
      this.truncating--
      this.state.mutex.unlock()
