...
module.exports = class Core {
  ...
  async truncate (length, fork, { signature, keyPair = this.header.keyPair } = {}) {
    ...
    try {
      ...
      batch.signature = signature || this.verifier.sign(batch.signable(), keyPair)
  ...
  async append (values, { signature, keyPair = this.header.keyPair } = {}, hooks = {}) {
    ...
    try {
      ...
      batch.signature = signature || await this.verifier.sign(this._legacy ? batch.signableLegacy(hash) : batch.signable(hash), keyPair)
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    const verifier = this.verifier || Core.createVerifier({ ...this.header, manifest }, { compat })
