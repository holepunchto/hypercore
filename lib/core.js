...
module.exports = class Core {
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.tree.length, treeLength = state.flushedLength(), overwrite = false, atomizer } = {}) {
    if (atomizer) atomizer.enter()

    let srcLock = null
    const lock = await this.state.mutex.lock(atomizer)
    ...
    try {
      ...
      const tree = await this.state._overwrite(state, length, treeLength, signature, false, atomizer)
