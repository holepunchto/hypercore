...
const { STORAGE_EMPTY, STORAGE_CONFLICT, INVALID_SIGNATURE } = require('hypercore-errors')
...
module.exports = class Core {
  ...
  static createAuth (header) {
    ...
    return createAuth(header.manifest, {
      keyPair,
      compat: isCompatHeader(header)
    })
  }
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, opts) {
    ...
    if (!header || overwrite) {
      ...
      const needsManifest = opts.key && (!keyPair || !b4a.equals(keyPair.publicKey, opts.key))
      const manifest = needsManifest ? null : createCompatManifest(keyPair.publicKey)
      const key = needsManifest ? (opts.key || caps.manifestHash(manifest)) : keyPair.publicKey
    ...
    const auth = opts.auth || (header.manifest ? this.createAuth(header) : null)
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    if (!this.header.manifest) {
      if (!manifest) { // compat mode, remove in future version
        manifest = createCompatManifest(this.header.key)
    ...
    const auth = this.defaultAuth || Core.createAuth({ ...this.header, manifest })
...
function createCompatManifest (publicKey) {
  return {
    hash: 'blake2b',
    static: null,
    signer: {
...
function isCompatHeader (header) {
  return !!(header.manifest && header.manifest.signer && b4a.equals(header.key, header.manifest.signer.publicKey))
}
