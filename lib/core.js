...
module.exports = class Core {
  ...
  createUserDataStream (start, end, session = this.state) {
    if (end instanceof SessionState) return this.createUserDataStream(start, null, end)
    return session.storage.createUserDataStream(start, end)
  }
  ...
  async audit () {
    const lock = await this.state.mutex.lock()
  ...
  async copyPrologue (src) {
    ...
    let srcLock = null
    try {
      srcLock = await src.mutex.lock()
    } catch (err) {
      this.state.mutex.unlock(lock)
      throw err
    }
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, length = state.length, treeLength = state.flushedLength(), overwrite = false } = {}) {
    let srcLock = null
    const lock = await this.state.mutex.lock()
    ...
    try {
      ...
      const tree = await this.state._overwrite(state, this.state.fork, length, treeLength, signature, false, false)
