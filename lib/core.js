...
module.exports = class Core {
  ...
  async _open(opts) {
    ...
    // always make sure we get an initial update
    if (this.state.length === 0 && !isWritableHeader(this.header)) {
      this.replicator.setDownloading(true)
      this.replicator.setDownloading(false)
    }
  }
  ...
  async _verifyShared() {
    ...
    try {
      ...
      if (verifies.length > 0) this.replicator.cork()
      for (let i = 0; i < verifies.length; i++) {
      ...
      if (verifies.length > 0) this.replicator.uncork()
...
function isWritableHeader(header) {
  return !!(header && header.keyPair && header.keyPair.secretKey)
}
