...
class SessionState {
  ...
  flushedLength () {
    ...
    const length = this.storage.dependencyLength()

    return length === -1 ? this.tree.length : length
  }
  ...
  async _overwrite (update, source, length, treeLength) {
    let batch = this.tree.batch()

    // truncate existing tree
    if (treeLength < this.tree.length) {
      this.blocks.clear(update.batch, length, this.tree.length)
      update.bitfield.setRange(length, this.tree.length, false)

      const bitfield = { start: length, length: this.tree.length - length, drop: true }
      const status = 0b0010

      update.coreUpdate({ status, bitfield, value: null, from: null })

      batch = await this.tree.truncate(treeLength)
    }
    ...
    if (treeLength < length) {
      ...
      const upgrade = { start: batch.length, length: length - treeLength }
      const proof = await source.tree.proof(reader, { upgrade })

      reader.tryFlush()

      const values = await Promise.all(promises)
      const upgradeProof = await proof.settle()

      this.blocks.putBatch(update.batch, treeLength, values)
      update.bitfield.setRange(treeLength, length, true)

      batch.verifyUpgrade(upgradeProof)
    ...
    if (batch.length < this.flushedLength()) {
      update.updateDependency(batch.length)
    }
  ...
  async overwrite (state, { length = state.tree.length, treeLength = state.flushedLength() } = {}) {
    ...
    try {
      const update = this.createUpdate()
      const batch = await this._overwrite(update, state, length, treeLength)
...
module.exports = class Core {
  ...
  async _getTreeHeadAt (length) {
    ...
    const rootHash = this.crypto.tree(roots)
  ...
  _updateSkipBitfield (ranges) {
    for (const { start, end, value } of ranges) {
      this.skipBitfield.setRange(start, end - start, value)
    }
  }
