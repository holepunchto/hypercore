...
class SessionState {
  ...
  ontruncate (to, from, fork) {
    if (this.isDefault()) {
      this.core.replicator.ontruncate(to, from - to)
      this.core.replicator.onupgrade()
...
module.exports = class Core {
  ...
  emitDownload (index, byteLength, from) {
    for (let i = this.monitors.length - 1; i >= 0; i--) {
      this.monitors[i].emit('download', index, byteLength, from)
    }
  }
  ...
  async _verifyExclusive ({ batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (batch.upgraded) this.state.onappend()
    } finally {
  ...
  _onupdate ({ status, bitfield, value, from }) {
    ...
    if (sessions.length === 0 || this.replicator === null) return
    ...
    if (value) {
      ...
      for (let i = 0; i < sessions.length; i++) {
        sessions[i].emit('download', bitfield.start, byteLength, from)
      }
    }
  }
  ...
  async _onconflict (proof, from) {
    ...
    for (const s of this.allSessions()) s.emit('conflict', proof.upgrade.length, proof.fork, proof)
