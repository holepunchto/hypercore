...
const createVerifier = require('./manifest')

module.exports = class Core {
  constructor (header, crypto, oplog, tree, blocks, bitfield, verifier, legacy, onupdate, onconflict) {
  ...
  static createVerifier (header, opts = {}) {
    if (!header.manifest) throw new Error('Needs manifest')

    const keyPair = opts.keyPair || {}

    if (!opts.keyPair && header.manifest.signer) {
      keyPair.secretKey = header.keyPair && header.keyPair.secretKey
      keyPair.publicKey = header.manifest && header.manifest.signer.publicKey
    }

    return createVerifier(header.manifest, { ...opts, keyPair })
  }

  static defaultSignerManifest (publicKey) {
    return defaultSignerManifest(publicKey)
  }
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, opts) {
    ...
    if (!header || overwrite) {
      ...
      const shouldCreateManifest = !opts.manifest || opts.compat || keyPair
      const manifest = shouldCreateManifest ? defaultSignerManifest(opts.key || keyPair.publicKey) : opts.manifest

      header = {
        key: opts.key || (opts.compat ? keyPair.publicKey : caps.manifestHash(manifest)),
        manifest,
        keyPair,
        userData: [],
        tree: {
    ...
    } else if (!header.manifest && opts.manifest) {
      if (opts.compat && header.key && !b4a.equals(header.key, opts.manifest.signer.publicKey)) {
        throw BAD_ARGUMENT('Key must match publicKey when in compat mode.')
      } else if (!b4a.equals(header.key, caps.manifestHash(opts.manifest))) {
        throw BAD_ARGUMENT('Manifest does not hash to provided key.')
      }
    ...
    const verifier = header.manifest ? this.createVerifier(header, opts) : null
    ...
    return new this(header, crypto, oplog, tree, blocks, bitfield, verifier, !!opts.legacy, opts.onupdate || noop, opts.onconflict || noop)
  }
  ...
  async copyFrom (src, signature, sign) {
    this._mutex.lock()
    ...
    try {
      ...
      this.tree.signature = signature || (sign && this.defeaultAuth.sign(this.tree.signable(), sign))
  ...
  async truncate (length, fork, { signature, keyPair = this.header.keyPair } = {}) {
    ...
    try {
      ...
      batch.signature = signature || this.defaultAuth.sign(batch.signable(), keyPair)
  ...
  async append (values, { signature, keyPair = this.header.keyPair } = {}, hooks = {}) {
    await this._mutex.lock()
    ...
    try {
      ...
      batch.signature = signature || await this.defaultAuth.sign(this._legacy ? batch.signableLegacy(hash) : batch.signable(hash), keyPair)
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    const compat = manifest && b4a.equals(this.header.key, manifest.signer.publicKey) // only need if manifest is present
    const verifier = this.defaultAuth || Core.createVerifier({ ...this.header, manifest }, { compat })

    if (!batch.signature || !verifier.verify(signable, batch.signature, batch)) {
