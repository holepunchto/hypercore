...
module.exports = class Core {
  ...
  async copyFrom (src, signature, { length = src.tree.length, additional = [] } = {}) {
    ...
    try {
      ...
      if (length > this.tree.length) {
        const missing = length - this.tree.length
        ...
        await this.blocks.putBatch(this.tree.length, blocks, this.tree.byteLength)
        this.bitfield.setRange(this.tree.length, missing, true)
      ...
      try {
        ...
        if (batch.length > this.tree.length) await batch.commit()
      } catch (err) {
