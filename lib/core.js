...
module.exports = class Core {
  ...
  async createSession (name, length, overwrite) {
    ...
    const currentTree = await this._getTreeHeadAt(length === -1 ? this.tree.length : length)

    if (!overwrite) {
      storage = await this.storage.openBatch(name)

      if (storage !== null) {
        prevTree = (await getCoreHead(storage)) || getDefaultTree()
        if (length !== -1 && prevTree.length !== length) throw STORAGE_CONFLICT('Different batch stored here')
      }
    }

    if (storage === null) {
      storage = await this.storage.registerBatch(name, currentTree)
    }

    const bitfield = await Bitfield.open(storage)

    bitfield.merge(this.bitfield, currentTree.length)

    const tree = await MerkleTree.open(storage, {
      crypto: this.crypto,
      prologue: this.tree.prologue,
      length: (length !== -1 || !prevTree) ? currentTree.length : prevTree.length
    })

    const sharedLength = prevTree
      ? Math.min(currentTree.length, prevTree.length)
      : currentTree.length
