...
module.exports = class Core {
  ...
  async _tryOpen (opts) {
    ...
    const treeInfo = {
      ...
      roots: header.tree.length ? await MerkleTree.getRoots(storage, header.tree.length) : [],
      prologue
    }
    ...
    this.state = new SessionState(this, null, storage, this.blocks, treeInfo, null)
  ...
  async _validateCommit (state, treeLength) {
    ...
    if (this.state.length > treeLength) {
      for (const root of this.state.roots) {
        const batchRoot = await MerkleTree.get(state.storage, root.index)
  ...
  async checkConflict (proof, from) {
    ...
    const batch = MerkleTree.verifyFullyRemote(proof, this.state)
    ...
    const localTreeHash = crypto.tree(await MerkleTree.getRoots(this.storage, proof.upgrade.length))
  ...
  async verifyReorg (proof) {
    ...
    await MerkleTree.reorg(this.storage, proof, batch)
    this._verifyBatchUpgrade(batch, proof.manifest)
    return batch
  }
  ...
  async verify (proof, from) {
    ...
    const batch = await MerkleTree.verify(this.storage, proof, this.state)
    if (!batch.commitable()) return false
