...
class Update {
  constructor (batch, bitfield, header) {
    this.batch = batch
  ...
  setBitfield (index, value) {
    this.bitfield.setRange(index, index + 1, value)
  }

  setBitfieldRange (start, length, value) {
    this.bitfield.setRange(start, start + length, value)
  }

  async flushBitfield () {
    const update = await this.bitfield.flush(this.batch)
    if (update) this._updates.push({ type: BITFIELD, update })
  }

  flushTreeBatch (batch) {
    const update = batch.commit(this.batch)
    this._updates.push({ type: TREE, update })
  ...
  flushCoreUpdate (update) {
    this._updates.push({ type: CORE, update })
  }

  coreUpdate (update) {
    this._coreUpdates.push(update)
  }

  processCoreUpdate ({ bitfield, status, value, from }) {
    const contig = updateContigBatch(this.contiguousLength, bitfield, this.bitfield)

    status |= contig.status

    if (contig.length > this.contiguousLength) {
      this.contiguousLength = contig.length
      this._updates.push({ type: CONTIG, update: contig.length })
    }

    this._updates.push({ type: CORE, update: { status, bitfield, value, from } })
  }
  ...
  async flush () {
    ...
    if (this.tree) {
      ...
      this._updates.push({ type: HEAD, update: this.tree })
    }

    for (const upd of this._coreUpdates) {
      this.processCoreUpdate(upd)
    }
...
module.exports = class Core {
  ...
  async audit () {
    ...
    try {
      const writer = this._createWriteBatch()
  ...
  _setManifest (writer, manifest, keyPair) {
    ...
    this._flushCoreUpdate({ status: 0b10000, bitfield: null, value: null, from: null })
  }
  ...
  get isFlushing () {
    return !!(this._flushing || this._activeBatch)
  }

  async _flushBitfield (writer) {
    if (!this._bitfield) return
    const update = await this._bitfield.flush(writer)
    if (update) this._updates.push({ type: BITFIELD, update })
  }

  async _flushTreeBatch (writer, batch) {
    const update = batch.commit(this._activeBatch)
    this._updates.push({ type: TREE, update })
  }

  async _flushCoreUpdate (update) {
    this._updates.push({ type: CORE, update })
  }

  async _flushCoreHead (writer, update) {
    writer.setCoreHead(update)
    this._updates.push({ type: HEAD, update })
  }

  async _flushUpdateBatch (u) {
    const updates = await u.flush()
    await this._flushUpdates(updates)
  }
  ...
  _createUpdate () {
    const writer = this._createWriteBatch()
    return new Update(writer, this.bitfield, this.header)
  }
  ...
  async _flushUpdate (u) {
    ...
    if (this._onflush) this._onflush()

    this._onflush = null
    this._flushing = null
  }

  async _flushWriter (w) {
    assert(w === this._activeBatch)

    const flushing = this._flushBatch(w)
  ...
  async clearBatch () {
    ...
    try {
      ...
      this._flushTreeBatch(writer, batch)
      await this._flushBitfield(writer)
  ...
  async clear (start, end, cleared) {
    ...
    try {
      ...
      await this._flushBitfield(writer)
      this._flushCoreUpdate({ status: 0, bitfield, value: null, from: null })
  ...
  //     const status = (batch.upgraded ? 0b0001 : 0) | updateContig(this.header, bitfield, this._bitfield)
  //     if (!pending) {
  ...
  //     if (this._shouldFlush()) await this._flushBitfield(writer)
  //   } finally {
  //     this._mutex.unlock()
  //   }
  ...
  async append (values, { signature, keyPair = this.header.keyPair, preappend } = {}) {
    ...
    try {
      const update = this._createUpdate()

      // upsert compat manifest
      if (this.verifier === null && keyPair) this._setManifest(update.batch, null, keyPair)
      ...
      const byteLength = this._appendBlocks(update.batch, values)
  ...
  async _verifyExclusive ({ batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      if (bitfield) {
        ...
        status |= updateContig(this.header, bitfield, this._bitfield)
      }
      ...
      this._flushCoreHead(writer, tree)

      this._flushCoreUpdate({ status, bitfield, value, from })
  ...
  async _truncate (writer, batch, from) {
    ...
    const contigStatus = updateContig(this.header, bitfield, this._bitfield)
    const status = ((batch.length > batch.ancestors) ? 0b0011 : 0b0010) | contigStatus

    this._flushCoreUpdate({ status, bitfield, value: null, from })
  ...
  _setBitfield (index, value) {
    if (!this._bitfield) this._bitfield = new BitInterlude(this.bitfield)
    this._bitfield.setRange(index, index + 1, value)
    if (this.skipBitfield !== null) this.skipBitfield.set(index, value)
  }

  _setBitfieldRange (start, length, value) {
    if (!this._bitfield) this._bitfield = new BitInterlude(this.bitfield)
    this._bitfield.setRange(start, start + length, value)
