  ...
  async copyFrom (src, upgrade) {
    ...
    try {
      ...
      while (pos >= 0) {
        ...
        this.blocks.putBatch(segmentStart, segment, offset)
      ...
      this.header.tree.rootHash = b4a.from(src.header.tree.rootHash)
      this.header.tree.signature = null
      this.header.userData = src.header.userData // should copy?

      if (upgrade) {
        const batch = await this.tree.verify(upgrade)
        if (!batch.commitable()) return false

        const hash = batch.hash()
        if (!batch.signature || !this._signed(batch, hash)) {
          // TODO: how to handle this upgrade failure?
          throw INVALID_SIGNATURE('Clone was provided an invalid upgrade')
        }
      }
