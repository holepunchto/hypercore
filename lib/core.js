...
module.exports = class Core {
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, opts) {
    ...
    if (!header || overwrite) {
      ...
      const noManifest = !opts.manifest && (!!opts.compat || !!keyPair)
      const manifest = noManifest ? defaultSignerManifest(opts.key || keyPair.publicKey) : opts.manifest
  ...
  async copyFrom (src, signature) {
    ...
    try {
      ...
      try {
        this._verifyBatchUpgrade(this.tree.batch(), this.header.manifest)
      } catch (err) {
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    const compat = manifest && manifest.signer && b4a.equals(this.header.key, manifest.signer.publicKey) // only need if manifest is present
