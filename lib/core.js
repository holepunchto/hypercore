...
module.exports = class Core {
  ...
  static createAuth (crypto, header) {
    ...
    const publicKey = header.manifest && header.manifest.signer.publicKey
  ...
  async copyFrom (src, signature, auth = this.defaultAuth) {
    ...
    try {
      ...
      if (signature && !this._verifyBatch(this.tree, null)) {
  ...
  async _verifyExclusive ({ batch, bitfield, value, from, manifest }) {
    this._verifyBatch(batch, manifest)

    // TODO: move this to tree.js
    const hash = batch.hash()
    if (!batch.signature || !this._signed(batch, hash, auth)) {
      throw INVALID_SIGNATURE('Proof contains an invalid signature')
    }
    ...
    try {
      ...
      const writingManifest = !this.header.manifest
      ...
      if (this._shouldFlush() || writingManifest) await this._flushOplog()
    } finally {
      this._mutex.unlock()
    }
  ...
  async checkConflict (proof, from) {
    ...
    this._verifyBatch(batch, proof.upgrade && proof.upgrade.manifest)
  ...
  async verify (proof, from) {
    ...
    const op = {
      ...
      value,
      from,
      manifest: proof.upgrade && proof.upgrade.manifest
    }
