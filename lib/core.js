...
const { manifestHash, createVerifier } = require('./manifest')
...
module.exports = class Core {
  ...
  static async resume (oplogFile, treeFile, bitfieldFile, dataFile, headerFile, opts) {
    ...
    if (!header || overwrite) {
      ...
      header = {
        external: null,
        key: opts.key || (opts.compat ? manifest.signer.publicKey : manifestHash(manifest)),
        manifest,
        keyPair,
        userData: [],
        tree: {
      ...
      if (opts.compat && header.key && !b4a.equals(header.key, opts.manifest.signer.publicKey)) {
        throw BAD_ARGUMENT('Key must match publicKey when in compat mode')
      } else if (!b4a.equals(header.key, manifestHash(opts.manifest))) {
    ...
    const verifier = header.manifest ? createVerifier(header.manifest, opts) : null
  ...
  _verifyBatchUpgrade (batch, manifest) {
    ...
    if (!this.header.manifest) {
      ...
      } else if (!manifest || !b4a.equals(this.header.key, manifestHash(manifest))) {
    ...
    const verifier = this.verifier || createVerifier(manifest, { compat })
