...
module.exports = class Core {
  ...
  _setManifest (writer, manifest, keyPair) {
    ...
    this._addUpdate(writer, { type: 0b001, entry: { status: 0b10000, bitfield: null, value: null, from: null } })
  }
  ...
  //     batch.commit(writer)
  //     this._addUpdate(writer, { type: 0b010, entry: batch })
  ...
  async _verifyExclusive (writer, { batch, bitfield, value, manifest, from }) {
    ...
    try {
      ...
      this._addUpdate(writer, { type: 0b001, entry: { status, bitfield, value, from } })
    } finally {
      this.updating = false
      this._mutex.unlock()
    }
  ...
  async _truncate (writer, batch, from) {
    ...
    this._addUpdate(writer, { type: 0b001, entry: { status, bitfield, value: null, from } })
  }
