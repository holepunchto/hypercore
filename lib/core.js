...
module.exports = class Core {
  constructor (storage, header, compat, crypto, tree, blocks, bitfield, verifier, sessions, legacy, globalCache, onupdate, onconflict) {
    ...
    this.state = { storage, tree, bitfield }
  ...
  async createNamedSession (name, length) {
    const storage = await this.storage.registerBatch(name, length)
  ...
  async _flushUpdateBatch (u) {
    ...
    if (u.state === this.state) await this._flushUpdates(updates)
  }
  ...
  _createUpdate (state) {
    ...
    this._activeBatch = state.storage.createWriteBatch()

    return new Update(this._activeBatch, state.bitfield, this.header, state)
  }
  ...
  async commit (state, { signature, keyPair = this.header.keyPair, treeLength = state.treeLength } = {}) {
    await this._mutex.lock()
    ...
    try {
      ...
      const adding = state.tree.length - treeLength

      const batch = await this.tree.reconcile(state.tree)

      batch.upgraded = batch.length > this.tree.length
      batch.treeLength = treeLength
      ...
      update.bitfield.setRange(bitfield.start, bitfield.length, true)
      update.flushTreeBatch(batch)
  ...
  async append (state, values, { signature, keyPair = this.header.keyPair, preappend } = {}) {
    ...
    try {
      ...
      this._appendBlocks(update.batch, values)
