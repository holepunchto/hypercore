...
class Peer {
  ...
  broadcastRange (start, length, drop, { force = false } = {}) {
  ...
  onopen ({ seeks, capability }) {
    ...
    if (contig > 0) {
      this.broadcastRange(0, contig, false, { force: true })
...
module.exports = class Replicator {
  ...
  get hasFullCoreLocally () {
    if (!this.core.state) return false
    if (!this.core?.header?.hints?.contiguousLength) return false
    return this.core.state.length === this.core.header.hints.contiguousLength
  }

  _ondata (peer, req, data) {
    if (!this.hasFullCoreLocally && this.fullyDownloaded) this.fullyDownloaded = false
    ...
    // When we detect we now have the core fully: notify all peers
    if (this.fullyDownloaded) return // they already know
    if (!this.hasFullCoreLocally) return
    this.fullyDownloaded = true
    for (const p of this.peers) {
      p.broadcastRange(0, this.core.state.length, false, { force: true })
    }
  }
