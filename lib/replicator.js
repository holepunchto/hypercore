...
const DEFAULT_MAX_INFLIGHT = [32, 256]
...
class Peer {
  ...
  getMaxInflight () {
    ...
    if (!stream.udx) return (this.inflightRange[0] + this.inflightRange[1]) / 2

    const ratio = Math.max(0, Math.min(1, (stream.rtt - 50) / 500))
    return this.inflightRange[0] + ratio * (this.inflightRange[1] - this.inflightRange[0])
  }
