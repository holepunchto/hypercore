...
class Peer {
  ...
  async _getProof (batch, msg) {
    ...
    if (msg.block) {
      ...
      if (msg.fork !== this.core.state.fork || !this.replicator.localBitfield.get(index)) {
  ...
  _clearLocalRange (start, length) {
    const bitfield = this.replicator.skipBitfield === null ? this.replicator.localBitfield : this.replicator.skipBitfield
...
module.exports = class Replicator {
  ...
  onhave (start, length, drop = false) {
    this._setBitfieldRanges(start, start + length, !drop)
    this.core.updateContiguousLength({ start, length, drop })
  ...
  async _resolveBlocksLocally () {
    ...
    for (const b of this._blocks) {
      if (this.localBitfield.get(b.index) === false) continue
  ...
  _setBitfieldRanges (start, end, value) {
    this.localBitfield.setRange(start, end, value)
  ...
  _unmarkInflight (index) {
    if (this.skipBitfield !== null) this.skipBitfield.set(index, this.localBitfield.get(index))
  ...
  _onwant (peer, start, length) {
    ...
    for (const msg of this.localBitfield.want(start, length)) {
...
function clampRange (core, r) {
  if (r.blocks === null) {
    const start = r.replicator.localBitfield.firstUnset(r.start)
    ...
    if (r.end === -1) r.start = start === -1 ? core.state.length : start
      ...
      const end = r.replicator.localBitfield.lastUnset(r.end - 1)

      if (end === -1 || start >= end + 1) r.end = r.start
      else r.end = end + 1
    }
  } else {
    while (r.start < r.end && r.replicator.localBitfield.get(r.blocks[r.start])) r.start++
    while (r.start < r.end && r.replicator.localBitfield.get(r.blocks[r.end - 1])) r.end--
  }
}
