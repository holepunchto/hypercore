...
class Peer {
  ...
  broadcastRange(start, length, drop) {
    ...
    let force = false
    if (fullyContig && !drop && !this.pushing) {
      start = 0
  ...
  async push(index) {
    ...
    msg.block = {
      index,
      nodes: MerkleTree.maxMissingNodes(2 * index, this.core.state.length)
    }
  ...
  async _fulfillRequest(req, pushing) {
    ...
    if (proof.block !== null) {
      ...
      // lets note that the remote peer has it for now, so we dont also HAVE it
      if (pushing) {
        this.pushing = true
        this.remoteBitfield.set(proof.block.index, true)
      }
    }
