...
class UpgradeRequest extends Attachable {
  ...
  _unref() {
    if (this.replicator.eagerUpgrade === true || this.inflight.length > 0) return false
...
class SeekRequest extends Attachable {
  ...
  _unref() {
    if (this.inflight.length > 0) return false
...
class Peer {
  ...
  async broadcastRange(start, length, drop) {
    if (!this.isActive()) return

    if (drop) await this._unclearLocalRange(start, length)
    else await this._clearLocalRange(start, length)
  ...
  async onopen({ seeks, capability }) {
    ...
    let resolveFullyOpening
    const promise = new Promise((resolve) => {
      resolveFullyOpening = resolve
    })
    this.remoteFullyOpening = promise
    ...
    if (contig > 0) {
      await this.broadcastRange(0, contig, false)
  ...
  async onclose(isRemote) {
    ...
    if (this.remoteFullyOpening) await this.remoteFullyOpening
    this.remoteOpened = false
    this.removed = true
  ...
  async onsync({ fork, length, remoteLength, canUpgrade, uploading, downloading, hasManifest }) {
    ...
    await this.replicator.localBitfield.lock() // Ensure the remote bitfield is updated
    ...
    this.replicator.localBitfield.unlock()
  ...
  async _getProof(batch, msg) {
    ...
    if (msg.block) {
      ...
      if (msg.fork !== this.core.state.fork || !(await this.replicator.localBitfield.get(index))) {
  ...
  async ondata(data) {
    ...
    try {
      ...
      safetyCatch(err)
      if (isBlockRequest(req)) await this.replicator._unmarkInflight(req.block.index)
    ...
    this.dataProcessing++
    await this.replicator.localBitfield.lock()
    ...
    await this.replicator._ondata(this, req, data)

    if (this._shouldUpdateCanUpgrade() === true) {
      await this._updateCanUpgradeAndSync()
    }
  ...
  async onwant({ start, length }) {
    ...
    await this.replicator.localBitfield.lock()
    await this.replicator._onwant(this, start, length)
    this.replicator.localBitfield.unlock()
  }
  ...
  async onbitfield({ start, bitfield }) {
    ...
    await this._clearLocalRange(start, bitfield.byteLength * 8)
    this._update()
  }

  async _clearLocalRange(start, length) {
    const bitfield =
      this.replicator.skipBitfield === null
        ? this.replicator.localBitfield
        : this.replicator.skipBitfield

    if (length === 1) {
      this.missingBlocks.set(start, this._remoteHasBlock(start) && !(await bitfield.get(start)))
      return
    }
    ...
    while (start < end) {
      ...
      const localBuf = local.bitfield ?? (await local.getBuffer())

      if (local && localBuf) {
        this.missingBlocks.clear(start, localBuf)
      }
  ...
  async _resetMissingBlock(index) {
    const bitfield =
      this.replicator.skipBitfield === null
        ? this.replicator.localBitfield
        : this.replicator.skipBitfield
    this.missingBlocks.set(index, this._remoteHasBlock(index) && !(await bitfield.get(index)))
  }

  async _unclearLocalRange(start, length) {
    if (length === 1) {
      await this._resetMissingBlock(start)
      return
    }
    ...
    await this._clearLocalRange(fixedStart, length)
  }
  ...
  async onrange({ drop, start, length }) {
    ...
    if (start === 0 && drop === false) {
      ...
      this.missingBlocks.set(start, has && !(await bitfield.get(start)))
    } else {
      ...
      if (rangeStart !== -1 && rangeStart < rangeEnd) {
        ...
        if (has) await this._clearLocalRange(rangeStart, rangeEnd - rangeStart)
      }
    }
  ...
  _requestSeek(s) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (this.replicator.localBitfield.getMaybe(index) === true) continue
  ...
  _requestRangeBlock(index, length) {
    if (this.replicator.localBitfield.getMaybe(index) === true || !this._canRequest(index))
      return false
  ...
  _findNext(i) {
    if (i < this._remoteContiguousLength) {
      if (this.replicator.skipBitfield === null) this.replicator._openSkipBitfield()
      i = this.replicator.skipBitfield.findFirst(false, i)
...
module.exports = class Replicator {
  ...
  async onhave(start, length, drop = false) {
    await this._setBitfieldRanges(start, start + length, !drop)
    await this.core.updateContiguousLength({ start, length, drop })
    for (const peer of this.peers) await peer.broadcastRange(start, length, drop)
  }

  // Called externally when a truncation upgrade has been processed
  async ontruncate(newLength, truncated) {
    const notify = []
    ...
    for (const peer of this.peers) await peer._unclearLocalRange(newLength, truncated)
  }

  // Called externally when a upgrade has been processed
  async onupgrade() {
    for (const peer of this.peers) peer.signalUpgrade()
    if (this._blocks.isEmpty() === false) await this._resolveBlocksLocally()
    if (this._upgrade !== null) this._resolveUpgradeRequest(null)
    if (!this._blocks.isEmpty() || this._ranges.length !== 0 || this._seeks.length !== 0) {
      await this._updateNonPrimary(true)
    }
  }
  ...
  async addRange(
    ...
    await clampRange(this.core, r)

    this._ranges.push(r)

    if (r.blocks !== null) {
      for (const block of r.blocks) {
        await this.localBitfield.ensurePage(block)
      }
    }
  ...
  async _resolveBlocksLocally() {
    ...
    for (const b of this._blocks) {
      if ((await this.localBitfield.get(b.index)) === false) continue
  ...
  async _updateNonPrimary(updateAll) {
    ...
    while (++this._updatesPending === 1) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        // TODO ensuring pages likely not required here because of clamp range call above
        if (r.blocks !== null) {
          for (const block of r.blocks) {
            await this.localBitfield.ensurePage(block)
          }
        ...
        if (r.end !== -1 && r.start >= r.end) {
          if (this._resolveRangeRequest(r)) i--
          if (this._ranges.length === MAX_RANGES) updateAll = true
        }
        if (len > this._ranges.length) len = this._ranges.length
      }
  ...
  _maybeResolveIfAvailableRanges() {
    ...
    for (let i = 0; i < this._ranges.length; i++) {
      ...
      if (r.ifAvailable && this._resolveRangeRequest(r)) i--
    }
  }
  ...
  _openSkipBitfield() {
    if (this.skipBitfield === null) {
      ...
      // TODO Fix this by preloading since otherwise the async will bubble up to the replication state machine
      const buf = this.localBitfield.toBuffer(this.core.state.length)
    ...
    for (const req of this._inflight) {
      if (req.block) this.skipBitfield.set(req.block.index, true) // skip
    }
  }

  async _setBitfieldRanges(start, end, value) {
    await this.localBitfield.setRange(start, end, value)
  ...
  _markInflight(index) {
    if (this.skipBitfield !== null) this.skipBitfield.set(index, true)
    for (const peer of this.peers) peer._markInflight(index)
  }

  async _unmarkInflight(index) {
    if (this.skipBitfield !== null)
      this.skipBitfield.set(index, await this.localBitfield.get(index))
    for (const peer of this.peers) await peer._resetMissingBlock(index)
  }
  ...
  async _ondata(peer, req, data) {
    ...
    if (this._seeks.length > 0 || this._ranges.length > 0)
      await this._updateNonPrimary(this._seeks.length > 0)
    this.updatePeer(peer)
  }

  async _onwant(peer, start, length) {
    ...
    for await (const msg of this.localBitfield.want(start, length)) {
  ...
  async destroy() {
    ...
    this.localBitfield.destroy() // TODO Decide if this is needed or useful
    ...
    while (this.peers.length) {
      ...
      await peer.channel.fullyClosed()
    }
...
async function clampRange(core, r) {
  if (r.blocks === null) {
    const start = await r.replicator.localBitfield.firstUnset(r.start)
    ...
    if (r.end === -1) r.start = start === -1 ? core.state.length : start
      ...
      const end = await r.replicator.localBitfield.lastUnset(r.end - 1)

      if (end === -1 || start >= end + 1) r.end = r.start
      else r.end = end + 1
    }
  } else {
    while (r.start < r.end && (await r.replicator.localBitfield.get(r.blocks[r.start]))) r.start++
    while (r.start < r.end && (await r.replicator.localBitfield.get(r.blocks[r.end - 1]))) r.end--
  }
}
