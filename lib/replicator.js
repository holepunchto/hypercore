...
class BlockRequest extends Attachable {
  constructor (tracker, index, priority) {
    ...
    this.activity = new ActivityQueue()
  }
  ...
  active () {
    const clock = this.activity.active()
    return clock
  }

  inactive (clock) {
    this.activity.inactive(clock)
    if (this.inflight.length === 0 && !this.activity.activity) {
      this.tracker.remove(this.index)
    }
  }

  drained () {
    return this.activity.drained()
  }
}
...
class Peer {
  ...
  async ondata (data) {
    ...
    if (isBlockRequest(req)) this.replicator._markActive(req)
    ...
    try {
      ...
      if (isBlockRequest(req)) {
        ...
        if (isBlockRequest(req)) this._unmarkActive(req)
      }
...
module.exports = class Replicator {
  ...
  flushBlock (index) {
    ...
    return b ? b.drained() : Promise.resolve(false)
  }
  ...
  clearRequests (session, err = null) {
    ...
    while (session.length > 0) {
      ...
      if (ref.activity) ref.activity.clear()
      cleared = true
    }
  ...
  _markActive (req) {
    const b = this._blocks.get(req.block.index)
    if (b === null) return
  ...
  _unmarkActive (req) {
    if (req.clock === null) return

    const b = this._blocks.get(req.block.index)
    if (b === null) return

    b.inactive(req.clock)
    req.clock = null
  }
