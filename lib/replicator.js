...
class BlockTracker {
  ...
  add(index, priority, force, debug) {
...
class Peer {
  ...
  sendSync() {
    ...
    this.replicator._log('hypercore-sync', {
      fork: this.core.state.fork,
      length: this.core.state.length,
      remoteLength: this.core.state.fork === this.remoteFork ? this.remoteLength : 0,
      canUpgrade: this.canUpgrade,
  ...
  async onsync(msg) {
    ...
    this.replicator._log('hypercore-onsync', msg)
  ...
  async push(index) {
    ...
    try {
      ...
      this.replicator._log('hypercore-push-error', {
        index,
        remoteLength,
        missingNodes: msg.block.nodes
      })

      return
    }

    this.replicator._log('hypercore-push', {
      index,
      remoteLength,
      pushedLength: this.pushedLength,
      remoteLength: this.remoteLength,
      pushable: this.remotePushable,
      synced: this.remoteSynced,
      missingNodes: msg.block.nodes,
      req
    })
  ...
  async _fulfillRequest(req, pushing) {
    ...
    if (proof.upgrade) {
      this.replicator._log('hypercore-fulfill', {
        length: this.core.state.length,
        pushedLength: this.pushedLength,
        remoteLength: this.remoteLength,
        proof
      ...
      if (req.msd.id === 0 && remoteLength > this.pushedLength) {
        this.pushedLength = remoteLength
      }
    }
  ...
  async _handleData(data) {
    ...
    } else if (this.replicator.pushOnly) {
      await this.replicator.mutex.lock()
    }
    ...
    try {
      if (data.upgrade) {
        this.replicator._log('hypercore-verify', {
          start: data.upgrade?.start,
          end: data.upgrade?.start + data.upgrade?.length,
          length: this.core.state.length,
          req: req ? { ...req, peer: null } : null,
          data
        })
      }
      ...
      if (data.upgrade) {
        this.replicator._log('hypercore-verify-success', {
          length: this.core.state.length,
          req: req ? { ...req, peer: null } : null,
          data
        })
      ...
      console.log(err)
      this.replicator._log('hypercore-verify-err', {
        err: err.message,
        start: data?.upgrade?.start,
        end: data?.upgrade?.start + data?.upgrade?.length,
        length: this.core.state.length,
  ...
  _makeRequest(needsUpgrade, priority, minLength, debug) {
  ...
  _requestBlock(b) {
    ...
    if (!this._canRequest(b.index, b.force)) {
      if (b.debug) console.log('bailing request (index, force, pushOnly)', b.index, b.force, this.replicator.pushOnly)
      return false
    }

    const req = this._makeRequest(b.index >= length, b.priority, b.index + 1, b.debug)
    if (b.debug) console.log('made request (index, req)', b.index, req)
    if (req === null) return false
  ...
  async _send(req) {
    ...
    try {
      if (req.block !== null && req.fork === fork) {
        ...
        if (req.debug) {
          console.log('replicator _send (req.block, state.length)', req.block, this.core.state.length)
          console.log('latest pushes (...indices)', ...this.replicator.pushes.slice(this.replicator.pushes.length - 8))
        }
...
module.exports = class Replicator {
  ...
  constructor(
    ...
    this.mutex = new Mutex() // used for push only mode
  ...
  _log (...args) {
    if (this.metrics !== null) {
      while (this.bufferedEvents.length) this.metrics.log(...(this.bufferedEvents.shift()))
      this.metrics.log(...args)
    } else {
  ...
  addBlock(session, index, force, debug) {
    if (debug) console.log('replicator addBlock (index, force)', index, force)
    const b = this._blocks.add(index, PRIORITY.HIGH, force, debug)
