...
class BlockRequest extends Attachable {
  constructor (tracker, index, priority) {
    ...
    this._activity = new ActivityQueue()
  }
  ...
  active () {
    const clock = this._activity.active()
    return clock
  }

  inactive (clock) {
    this._activity.inactive(clock)
    if (this.inflight.length === 0 && !this._activity.activity) {
      this.tracker.remove(this.index)
    }
  }

  drained () {
    return this._activity.drained()
  }
}
