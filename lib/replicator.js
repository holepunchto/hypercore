...
class Peer {
  ...
  async _handleRequest(msg) {
    ...
    if (!req.upgrade) {
      return this._fulfillRequest(req, false)
    }

    let locked = false
    await this.replicator._txLock.lock()
    this.replicator._log('hypercore-tx-lock', {
      msg,
      upgrade: req.upgrade
    })
    try {
      locked = true
      const end = proof.upgrade.start + proof.upgrade.length

      if (this.pushedLength < end) {
        req.upgrade.start = this.pushedLength
      } else {
        req.upgrade === null
        this.replicator._txLock.unlock()
        locked = false
      ...
      this.replicator._log('hypercore-tx-unlock', {
        msg,
        upgrade: req.upgrade
      })
  ...
  async _handleData(data) {
    ...
    if (data.upgrade) {
      ...
      this.replicator._log('hypercore-lock', {
    ...
    try {
      ...
      if (locked) this.replicator._rxLock.unlock()
      return
    }
    ...
    try {
      ...
      if (locked) {
        ...
        this.replicator._log('hypercore-unlock', {
          start: data.upgrade?.start,
          end: data.upgrade?.start + data.upgrade?.length,
          length: this.core.state.length,
        })
