...
class RequestPool {
  ...
  async ondata (proof, peer) {
    ...
    // if we get a message from another fork, ignore it, prob race condition
    if (peer.state.fork !== this.core.fork) return
...
module.exports = class Replicator {
  ...
  oninfo ({ length, fork }, peer) {
    ...
    if (forked) {
      // TODO: this can be done much more safely and better
      for (let i = peer.state.length; i < len; i++) {
        peer.bitfield.set(i, false)
      }
    }
