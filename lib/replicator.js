...
class Range {
  ...
  _done (err, done) {
    ...
    else this._ranges[i] = this._ranges.pop()
...
module.exports = class Replicator {
  ...
  requestRange (start, end, linear) {
    const range = this.requests.range(start, end, linear)
    this.updateAll()
    return range
  }
  ...
  async onrequest (req, peer) {
    ...
    const signature = this.core.oplog.signature
...
async function verifyAndWrite (core, proof, peer) {
  const batch = await core.tree.verify(proof)

  if (batch.upgraded && !batch.signedBy(core.oplog.publicKey)) {
    throw new Error('Remote signature does not match')
  }

  const { block } = proof
  const downloaded = !!block && !!block.value && !core.bitfield.get(block.index)

  const index = block ? block.index : 0
  const value = block && block.value

  if (batch.upgraded) {
    // TODO: Peer needs to be sent through the pipeline here
    await core.oplog.upgrade(batch, index, value)
  } else {
    await core.oplog.put(batch, index, value)
  }

  // TODO: Should be handled through an event propagating from the oplog
