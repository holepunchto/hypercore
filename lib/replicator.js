...
class Peer {
  ...
  getMaxInflight() {
    ...
    const scale =
      stream.rtt <= SCALE_LATENCY
        ? 1
        : (stream.rtt / SCALE_LATENCY) *
          Math.min(1, 2 / this.replicator.peers.length)
    return Math.max(
  ...
  broadcastRange(start, length, drop) {
    ...
    const contig =
      this.core.header.hints.contiguousLength === this.core.state.length
  ...
  extension(name, message) {
    this.wireExtension.send({
      name: name === this.lastExtensionSent ? '' : name,
      message
    })
  ...
  onextension(message) {
    ...
    if (ext)
      ext._onmessage(
        { start: 0, end: message.message.byteLength, buffer: message.message },
        this
      )
  }
  ...
  sendSync() {
    ...
    this.wireSync.send({
      ...
      length: this.core.state.length,
      remoteLength:
        this.core.state.fork === this.remoteFork ? this.remoteLength : 0,
  ...
  onopen({ seeks, capability }) {
    ...
    const contig = Math.min(
      this.core.state.length,
      this.core.header.hints.contiguousLength
    )
    if (contig > 0) {
  ...
  closeIfIdle() {
    if (
      this.remoteDownloading === false &&
      this.replicator.isDownloading() === false
    ) {
      // idling, shut it down...
      this.channel.close()
      return true
  ...
  async onsync({
    ...
    if (
      this.remoteLength > this.core.state.length &&
  ...
  async _updateCanUpgradeAndSync() {
    ...
    const canUpgrade = await this._canUpgrade(
      this.remoteLength,
      this.remoteFork
    )
  ...
  async _canUpgrade(remoteLength, remoteFork) {
    ...
    try {
      ...
      const canUpgrade = await MerkleTree.upgradeable(
        this.core.state,
        remoteLength
      )
  ...
  async _getProof(batch, msg) {
    ...
    const manifest =
      msg.manifest && !this.core.compat ? this.core.header.manifest : null
  ...
  async _fulfillRequest(req) {
    ...
    if (proof.block !== null) {
      this.replicator._onupload(
        proof.block.index,
        proof.block.value.byteLength,
        this
      )
    }
  ...
  async ondata(data) {
    ...
    const req =
      data.request > 0 ? this.replicator._inflight.get(data.request) : null
    ...
    try {
      if (
        !matchingRequest(req, data) ||
        !(await this.core.verify(data, this))
      ) {
  ...
  onwant({ start, length }) {
    ...
    if (this.remoteSegmentsWanted.size >= MAX_REMOTE_SEGMENTS)
      this.remoteSegmentsWanted.clear() // just in case of abuse
  ...
  onbitfield({ start, bitfield }) {
    if (start < this._remoteContiguousLength)
      this._remoteContiguousLength = start // bitfield is always the truth
  ...
  _clearLocalRange(start, length) {
    const bitfield =
      this.core.skipBitfield === null
        ? this.core.bitfield
        : this.core.skipBitfield

    if (length === 1) {
      this.missingBlocks.set(
        start,
        this._remoteHasBlock(start) && !bitfield.get(start)
      )
      return
    }
  ...
  _resetMissingBlock(index) {
    ...
    this.missingBlocks.set(
      index,
      this._remoteHasBlock(index) && !bitfield.get(index)
    )
  }
  ...
  _makeRequest(needsUpgrade, priority, minLength) {
    if (
      needsUpgrade === true &&
      this.replicator._shouldUpgrade(this) === false
    ) {
      return null
    }
  ...
  _canRequest(index) {
    ...
    if (
      index < this.lastUpgradableLength &&
      this.lastUpgradableFork === this.core.state.fork
    )
      return true
  ...
  _remoteHasBlock(index) {
    return (
      index < this._remoteContiguousLength ||
      this.remoteBitfield.get(index) === true
    )
  }
  ...
  _requestRangeBlock(index, length) {
    if (this.core.bitfield.get(index) === true || !this._canRequest(index))
      return false
  ...
  _requestRange(r) {
    ...
    if (r.blocks) {
      ...
      for (let i = r.start; i < r.end; i++) {
        ...
        const has =
          index < this._remoteContiguousLength ||
          this.missingBlocks.get(index) === true
...
module.exports = class Replicator {
  ...
  _checkUpgradeIfAvailable() {
    ...
    for (let i = 0; i < maxPeers; i++) {
      ...
      if (
        peer.remoteLength <= this._upgrade.length ||
        peer.remoteFork !== this._upgrade.fork
      )
        continue
  ...
  _addUpgrade() {
    ...
    this._upgrade = new UpgradeRequest(
      this,
      this.core.state.fork,
      this.core.state.length
    )
  ...
  _updatePeerNonPrimary(peer) {
    ...
    if (
      this._maybeUpdate() === true &&
      peer._requestUpgrade(this._upgrade) === true
    ) {
      return true
    }
  ...
  attachTo(protomux) {
    ...
    protomux.pair(
      { protocol: 'hypercore/alpha', id: this.core.discoveryKey },
      makePeer
    )
...
function matchingRequest(req, data) {
  if (
    data.block !== null &&
    (req.block === null || req.block.index !== data.block.index)
  )
    return false
  if (
    data.hash !== null &&
    (req.hash === null || req.hash.index !== data.hash.index)
  )
    return false
  if (
    data.seek !== null &&
    (req.seek === null || req.seek.bytes !== data.seek.bytes)
  )
    return false
...
function onwirerequest(m, c) {
  incrementRx(
    c.userData.stats.wireRequest,
    c.userData.replicator.stats.wireRequest
  )
...
function onwirecancel(m, c) {
  incrementRx(
    c.userData.stats.wireCancel,
    c.userData.replicator.stats.wireCancel
  )
...
function onwirebitfield(m, c) {
  incrementRx(
    c.userData.stats.wireBitfield,
    c.userData.replicator.stats.wireBitfield
  )
...
function onwireextension(m, c) {
  incrementRx(
    c.userData.stats.wireExtension,
    c.userData.replicator.stats.wireExtension
  )
