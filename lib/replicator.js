...
class Peer {
  ...
  async push(block){
    ...
    if (this.remoteBitfield.get(block)) return
    ...
    msg.block = {
      index: block,
      nodes: MerkleTree.maxMissingNodes(block, this.remoteLength)
    }

    if (block >= this.remoteLength) {
      msg.upgrade = {
