...
class Attachable {
  ...
  _detach (r) {
    ...
    if (r.context.inflight) {
      for (const req of r.context.inflight) {
        req.peer.wireCancel.send({ request: req.id })
      }
    }
...
class Peer {
  constructor (replicator, protomux, channel, session) {
    ...
    this.receiverSemaphore = new Semaphore(2)
  ...
  async _drainRequests () {
    if (this.receiverSemaphore.active >= this.receiverSemaphore.limit) return

    while (this.receiverQueue.length > 0) {
      await this.receiverSemaphore.wait()

      try {
        const msg = this.receiverQueue.shift()
        if (msg) await this._drainRequest(msg)
      } finally {
        this.receiverSemaphore.signal()
      }
    }
  }

  async _drainRequest (msg) {
    let proof = null
