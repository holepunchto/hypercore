...
class Peer {
  constructor (replicator, protomux, channel, session, inflightRange, 
    ...
    this.name = name + ':Peer:' + randName()

    this.l = (...x) => log(this.name, ...x)
    this.tx_msgs ??= [];
    this.add_send = (name, msg) => {
      MSGS.push({name: this.name + name, ...msg});
      this.tx_msgs.push({name, ...msg});
      return this.l(name, msg);
    }
    ...
    this.remoteFork = 0
    // how long we think the peers core is
    this.remoteLength = 0
  ...
  broadcastRange (start, length, drop) {
   ...
   const msg = { 
    ...
    this.add_send(' :wireRange.send', msg);
    this.wireRange.send(msg);
  }

  extension (name, message) {
    const msg = { name: name === this.lastExtensionSent ? '' : name, message };
    this.add_send(' :wireExtension.send', msg);
    this.wireExtension.send(msg);
  ...
  sendSync () {
    ...
    this.add_send(' :wireSync.send', msg);
    this.wireSync.send(msg);
  }
  ...
  async _handleRequest (msg) {
    ...
    if (proof === null) {
      if (msg.manifest && this.core.header.manifest) {
        ...
        const out = { request: msg.id, fork: this.core.tree.fork, block: null, hash: null, seek: null, upgrade: null, manifest };
        this.add_send(' :wireData.send', out);
        this.wireData.send(out);
        return
      }

      const out2 = { request: msg.id };
      this.add_send(' :wireNoData.send', out2);
      this.wireNoData.send(out2);
      return
    }
    ...
    this.add_send(' :wireData.send', out3);
    this.wireData.send(out3)
  }
  ...
  _cancelRequest (id) {
    ...
    this.add_send(' :wireCancel.send', msg);
    this.wireCancel.send(msg);
  }
  ...
  _checkIfConflict () {
    ...
    this.add_send(' :wireRequest.send', msg);
    this.wireRequest.send(msg);
  }
  ...
  _maybeWant (start, length = 1) {
    ...
    for (; i < n; i++) {
      ...
      this.add_send(' :wireWant.send', msg);
      this.wireWant.send(msg);
    }
  }
  ...
  async _send (req) {
    ...
    this.add_send(' :wireRequest.send', {...req, ...{peer: '<hidden>'}});
...
module.exports = class Replicator {
  ...
  _onwant (peer, start, length) {
    ...
    for (const msg of this.core.bitfield.want(start, length)) {
      peer.add_send(' :wireBitfield.send', msg);
  ...
  _makePeer (protomux, session) {
    ...
    this.rx_msgs ??= [];
    let wrapFunc = (name, func) => {
      return (...x) => {
        this.rx_msgs.push({name, ...x[0]});
        this.l(':RX' + name + ' msg:\n', x[0]);
        func(...x)
      }
    }
    const channel = protomux.createChannel({
      ...
      messages: [
        { encoding: m.wire.sync, onmessage: wrapFunc(':onwiresync', onwiresync)},
        { encoding: m.wire.request, onmessage: wrapFunc(':onwirerequest', onwirerequest)},
        { encoding: m.wire.cancel, onmessage: wrapFunc(':onwirecancel', onwirecancel)},
        { encoding: m.wire.data, onmessage: wrapFunc(':onwiredata', onwiredata)},
        { encoding: m.wire.noData, onmessage: wrapFunc(':onwirenodata', onwirenodata)},
        { encoding: m.wire.want, onmessage: wrapFunc(':onwirewant', onwirewant)},
        { encoding: m.wire.unwant, onmessage: wrapFunc(':onwireunwant', onwireunwant)},
        { encoding: m.wire.bitfield, onmessage: wrapFunc(':onwirebitfield', onwirebitfield)},
        { encoding: m.wire.range, onmessage: wrapFunc(':onwirerange', onwirerange)},
        { encoding: m.wire.extension, onmessage: wrapFunc(':onwireextension', onwireextension)}
      ],
      onopen: onwireopen,
      onclose: onwireclose,
      ondrain: onwiredrain,
    })
      //onopen: wrapFunc(this.name + 'proto channel :onopen', onwireopen),
      //onclose: wrapFunc(this.name + 'proto channel :onclose', onwireclose),
      //ondrain: wrapFunc(this.name + 'proto channel :ondrain', onwiredrain),

    if (channel === null) return onnochannel()

    const peer = new Peer(replicator, protomux, channel, session, this.inflightRange, {name: this.name, log: this.l})
