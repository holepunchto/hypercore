...
class Peer {
  ...
  onclose (isRemote) {
    ...
    if (this.useSession && !reopen) this.replicator._closeSessionMaybe()
...
module.exports = class Replicator {
  ...
  _closeSessionMaybe () {
    if (this._hasSession && this._peerSessions === 0 && this._peerAttaching === 0) {
      this._hasSession = false
      this.core.active--
    }
  ...
  attachTo (protomux, useSession) {
    ...
    if (useSession) this._peerAttaching++
    this._ifAvailable++
