...
class Attachable {
  ...
  gc() {
    ...
    for (const w of wants) {
      if (w.wants.removeBatch(w.index, this)) {
...
class Peer {
  ...
  _requestSeek(s) {
    ...
    this._maybeWantRange(s.seeker.start, len)
    return false
  }
  ...
  _maybeWantRange(start, length, handle) {
  ...
  _addRangeBatch(r, offset, end) {
    ...
    let b = (next - (next & (WANT_BATCH - 1))) / WANT_BATCH
    while (r.batches.indexOf(b) > -1) b++

    const index = b * WANT_BATCH
  ...
  _populateRangeBatches(r) {
    for (let i = r.batches.length - 1; i >= 0; i--) {
      const b = r.batches[i]
      const sparse = this.core.bitfield.hasUnset(b * WANT_BATCH, WANT_BATCH)
      if (sparse) continue
      const head = r.batches.pop()
      if (head !== b) r.batches[i] = head
      this.wants.remove(b * WANT_BATCH, r)
    }
    ...
    while (r.batches.length < 3) {
      ...
      if (!this._addRangeBatch(r, off, end) && !this._addRangeBatch(r, 0, end)) break
    }
  }
  ...
  _requestRange(r) {
    ...
    for (let i = 0; i < r.batches.length; i++) {
      ...
      const batchStart = Math.max(b * WANT_BATCH, r.start)
      const batchEnd = Math.min(batchStart + WANT_BATCH, end)
      const len = batchEnd - batchStart
      const off = batchStart + (r.linear ? 0 : Math.floor(Math.random() * len))

      if (this._requestRangeBatch(off, end, length) || this._requestRangeBatch(batchStart, off, length)) {
        return true
      }

      this._maybeWant(batchStart, r)
    }
  ...
  _requestForkRange(f) {
    ...
    this._maybeWantRange(f.batch.want.start, len, f)
    return false
  }
