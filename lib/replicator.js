...
class Peer {
  ...
  async onrequest (msg) {
    ...
    const req = await this._handleRequest(batch, msg)
  ...
  async ondrain () {
    const batch = this.core.storage.createReadBatch()
    const requests = await this._handleRequests(batch)
  ...
  async _handleRequests () {
    ...
    while (this.remoteOpened && this.protomux.drained && this.receiverQueue.length > 0 && !this.removed) {
      ...
      proofs.push(await this._handleRequest(msg))
    }

    return proofs
  }

  async _handleRequest (batch, msg) {
    // TODO: could still be answerable if (index, fork) is an ancestor of the current fork
    if (msg.fork !== this.core.tree.fork) return Promise.resolve(ProofRequest(msg, null, null, null))

    return this._getProof(batch, msg)
  }
