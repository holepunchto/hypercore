...
class Peer {
  ...
  sendSync() {
    ...
    console.log(Date.now(), 'sending sync', {
  ...
  set remoteAllowPush(allowed) {
    const stack = new Error().stack
    console.log('setting remoteAllowPush', allowed)
    this._remoteAllowPush = allowed
  }

  get remoteAllowPush() {
    return this._remoteAllowPush
  }
  ...
  async onsync(msg) {
    ...
    console.log(Date.now(), 'sync received!', msg, this.remoteAllowPush)
    ...
    console.log(Date.now(), 'sync processed', this.remoteAllowPush)
  ...
  async push(index) {
    console.log(Date.now(), 'push start...', index)
    try {
      console.log(Date.now(), this.remoteAllowPush)
      if (!this.remoteAllowPush) {
        console.log(Date.now(), 'push bail: not allowed', index, this.remoteAllowPush)
        return
      }
      console.log(Date.now(), 'push: remoteDownloading', index)
      console.log(Date.now(), this.remoteDownloading)
      if (!this.remoteDownloading) {
        console.log(Date.now(), 'push bail: not downloading', index, this.remoteDownloading)
        return
      }
      console.log(Date.now(), 'push: length check', index)
      console.log(Date.now(), this.core.state.length, index)
      if (this.core.state.length <= index) {
        console.log(Date.now(), 'push bail: no length', index, this.core.state.length)
        return
      }
      console.log(Date.now(), 'push: fork check', index)
      console.log(Date.now(), this.core.state.fork, this.remoteFork)
      if (this.core.state.fork !== this.remoteFork) {
        console.log(Date.now(), 'push bail: no fork', index, this.core.state.fork, this.remoteFork)
        return
      }
      console.log(Date.now(), 'push: bitfield check', index)
      console.log(Date.now(), this.remoteBitfield.get(index))
      if (this.remoteBitfield.get(index)) {
        console.log(Date.now(), 'push bail: remote has bit', index, this.remoteBitfield.get(index))
        return
      }
    } catch (err) {
    ...
    console.log(Date.now(), 'pushing...', index)
    ...
    try {
      ...
      console.log(Date.now(), 'fulfilling push...', index)
  ...
  async _fulfillRequest(req, pushing) {
    console.log(Date.now(), 'fulfilling request...', {
      block: req.block ? req.block.index : null,
      upgrade: req.upgrade ? req.upgrade.start + req.upgrade.length : null,
      hash: req.hash ? req.hash.index : null,
      manifest: req.manifest || false
    })
    ...
    console.log(Date.now(), 'fulfilled request!', {
      block: proof.block ? proof.block.index : null,
      upgrade: proof.upgrade ? proof.upgrade.start + proof.upgrade.length : null,
      hash: proof.hash ? proof.hash.index : null,
  ...
  async _handleData(data) {
    console.log(Date.now(), 'handling data', data)
    ...
    console.log(Date.now(), 'verifying data', data)
    try {
      ...
      console.log(Date.now(), 'verifying data error', err)
      safetyCatch(err)
      this._unmarkInflightBlockRequest(req, data)
...
module.exports = class Replicator {
  ...
  _onpeerupdate(added, peer) {
    ...
    console.log(Date.now(), 'peer-add', { publicKey: peer.remotePublicKey })
  ...
  _ondownload(index, byteLength, from, req) {
    ...
    console.log(Date.now(), 'download', { index, byteLength, req: { ...req, peer: null } })
  ...
  _onupload(index, byteLength, from) {
    ...
    console.log(Date.now(), 'upload', { index, byteLength })
  ...
  _oninvaliddata(err, req, res, from) {
    ...
    console.log(Date.now(), 'invalid-data', { err })
  ...
  _oninvalidrequest(err, req, from) {
    ...
    console.log(Date.now(), 'invalid-request', { err })
