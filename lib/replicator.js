...
class Peer {
  ...
  getMaxInflight () {
    ...
    this.$time = Date.now()
    const latency = !time ? 0 : this.$time - time

    console.log('latency', latency, 'stream', { rtt: stream.rtt, inflight: stream.inflight }, 'inflight (peer / max)', this.inflight + '/' + this.maxInflight)
