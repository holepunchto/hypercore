...
class Peer {
  ...
  onclose (isRemote) {
    ...
    if (this.remoteOpened === false) {
      const i = this.replicator.peers.indexOf(this)
      if (i > -1) console.log('not removing peer', i, this.replicator.peers.length)
...
module.exports = class Replicator {
  ...
  _addPeer (peer) {
    this._hadPeers = true
    const index = this.peers.push(peer) - 1
    console.log(this.$id, '_addPeer', index, '(' + this.peers.length + ')')
  ...
  _removePeer (peer) {
    ...
    console.log(this.$id, '_removePeer', index, '(' + this.peers.length + ')')
  ...
  _makePeer (protomux, session) {
    ...
    peer.$timeout = setTimeout(() => {
      console.log('peer still did not open either close')
    }, 30000)

    /* stream.once('close', function () {
      if (peer.$timeout === null) return
