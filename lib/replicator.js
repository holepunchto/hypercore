...
const INITIATOR = b4a.from([1])
const RESPONDER = b4a.from([2])
...
function cap (stream, key, inverse) {
  ...
  const type = stream.isInitiator !== inverse ? INITIATOR : RESPONDER

  sodium.crypto_generichash_batch(capability, [NS, type, key], stream.handshakeHash)
