...
class RangeRequest extends Attachable {
  constructor (ranges, start, end, linear, ifAvailable, blocks, priority) {
    super()
...
class SeekRequest extends Attachable {
  constructor (seeks, seeker, priority) {
    super()
...
class Peer {
  ...
  _requestSeek (s) {
    ...
    if (s.seeker.start >= length) {
      const req = this._makeRequest(true, s.priority)
    ...
    for (let i = 0; i < len; i++) {
      ...
      const h = this.replicator._hashes.add(index, s.priority)
  ...
  _requestRange (r) {
    ...
    for (let i = 0; i < len && wrapped < 2; i++) {
      ...
      const b = this.replicator._blocks.add(index, r.priority)
...
module.exports = class Replicator {
  ...
  addBlock (session, index, { priority = PRIORITY.HIGH } = {}) {
    const b = this._blocks.add(index, priority)
  ...
  addSeek (session, seeker, { priority = PRIORITY.NORMAL } = {}) {
    const s = new SeekRequest(this._seeks, seeker, priority)
  ...
  addRange (session, { start = 0, end = -1, length = toLength(start, end), blocks = null, linear = false, ifAvailable = false, priority = PRIORITY.NORMAL } = {}) {
