...
module.exports = class Replicator {
  ...
  onhave (start, length, drop = false) {
    ...
    if (!drop) {
      ...
      // Send the biggest range we can, rather than the incremental upgrade
      if (contig && contig > start) {
        start = 0
        length = contig
      }

      // Always send the broadcast when we switch from sparse to fully contiguous
      if (!this.fullyDownloaded && this.hasFullCore) {
        this.fullyDownloaded = true
        force = true
      }
    }

    for (const peer of this.peers) peer.broadcastRange(start, length, drop, { force })
  }
