  ...
  async _updateNonPrimary () {
    ...
    while (++this._updatesPending === 1) {
      for (let i = 0; i < this._ranges.length; i++) {
        ...
        if (r.blocks === null) {
          const start = this.core.bitfield.firstUnset(r.start)

          if (r.end === -1) {
            r.start = start === -1 ? this.core.tree.length : start
          } else {
            const end = this.core.bitfield.lastUnset(r.end - 1) + 1
          ...
          while (this.core.bitfield.get(r.blocks[r.start])) r.start++
        } else {
          while (r.start < r.end && this.core.bitfield.get(r.blocks[r.start])) r.start++
          while (r.start < r.end && this.core.bitfield.get(r.blocks[r.end - 1])) r.end--
        }
