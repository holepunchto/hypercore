...
class Peer {
  ...
  async _fulfillRequest (req) {
    ...
    if (proof.block !== null) {
      this.replicator._onupload(proof.block.index, proof.block.value, this)
    }
...
module.exports = class Replicator {
  ...
  _ondata (peer, req, data) {
    if (data.block !== null) {
      ...
      // this.emitDownload(data.block.index, data.block.value.byteLength, peer)
    }
  ...
  _onpeerupdate (added, peer) {
    ...
    const sessions = this.core.allSessions()
  ...
  _onupload (index, value, from) {
    const padding = this.core.encryption ? this.core.encryption.padding : 0
    const byteLength = value.byteLength - padding
    const sessions = this.core.allSessions()

    for (let i = 0; i < sessions.length; i++) {
      sessions[i].emit('upload', index, byteLength, from)
    }
  }
