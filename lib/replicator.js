...
module.exports = class Replicator {
  ...
  _updateRanges(index, limit) {
    if (this._ranges.length === 0) {
      return { checked: 0, resolved: 0, index: 0, updateAll: false }
    }
    ...
    while (this._updatesQueued) {
      ...
      while (true) {
        ...
        if (this._ranges.length > 0) {
          ...
          continueRanges = checked > 0 && checkedSinceResolve < this._ranges.length && drain
        }
        ...
        if ((!continueRanges || this._seeks.length > 0) && (this._inflight.idle || updateAll)) {
          this.queueUpdateAll()
        }
