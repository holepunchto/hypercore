...
module.exports = class Replicator {
  ...
  _updateRanges(index, limit) {
    if (this._ranges.length === 0) {
      return { checked: 0, resolved: 0, hitMaxAfterResolve: false, index: 0 }
    }
    ...
    while (remaining-- > 0 && this._ranges.length > 0) {
      ...
      if (r.end !== -1 && r.start >= r.end) {
        this._resolveRangeRequest(r)
        resolved++
        if (this._ranges.length === MAX_RANGES) hitMaxAfterResolve = true
      } else {
        index++
      }
    }
    ...
    return { checked, resolved, hitMaxAfterResolve, index }
  }
  ...
  async _updateNonPrimary(updateAll) {
    ...
    while (++this._updatesPending === 1) {
      ...
      while (this._ranges.length > 0) {
        ...
        const { checked, resolved, hitMaxAfterResolve } = result
        rangeIndex = result.index

        if (hitMaxAfterResolve) updateAll = true
        ...
        await yieldToLoop()
        if (this.destroyed || this._updatesPending > 1) break
      }
