...
module.exports = class Replicator {
  ...
  _ondata (peer, req, data) {
    req.elapsed = req.timestamp ? Date.now() - req.timestamp : 0
    if (data.block !== null) {
      this._resolveBlockRequest(this._blocks, data.block.index, data.block.value, req)
      this._ondownload(data.block.index, data.block.value.byteLength, peer, { elapsed: req.elapsed })
    }
  ...
  _ondownload (index, byteLength, from, metadata = {}) {
    const sessions = this.core.monitors

    for (let i = sessions.length - 1; i >= 0; i--) {
      const s = sessions[i]
      s.emit('download', index, byteLength - s.padding, from, metadata)
    }
  }
