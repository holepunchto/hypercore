...
class Peer {
  constructor (replicator, protomux, channel, session) {
    ...
    this.inflight = 0
    this.inflightRange = [DEFAULT_MAX_INFLIGHT, DEFAULT_MAX_INFLIGHT]
  ...
  getMaxInflight () {
    const maxInflight = (this.inflightRange[0] + this.inflightRange[1]) / 2

    const stream = this.stream.rawStream.stream || this.stream.rawStream
    if (!stream.udx) return maxInflight

    const ratio = Math.max(0, Math.min(1, (stream.rtt - 50) / 500))
    return maxInflight + Math.floor(256 * ratio)
  }
