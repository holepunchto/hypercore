...
class Peer {
  ...
  onrange({ drop, start, length }) {
    ...
    const storedRemoteContig = this.core.header.hints.remoteContiguousLength

    if (drop === false && start + length > storedRemoteContig && start <= storedRemoteContig) {
      this.core.updateRemoteContiguousLength(start + length).catch(noop)
    }
