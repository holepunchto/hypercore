...
class Peer {
  ...
  async _canUpgrade (remoteLength, remoteFork) {
    ...
    try {
      ...
      const canUpgrade = await MerkleTree.upgradeable(this.core.state.storage, remoteLength)
  ...
  async _getProof (batch, msg) {
    ...
    const proof = await MerkleTree.proof(batch, treeBatch, msg)
  ...
  async _send (req) {
    ...
    try {
      if (req.block !== null && req.fork === fork) {
        req.block.nodes = await MerkleTree.missingNodes(this.core.state.storage, 2 * req.block.index, this.core.state.length)
        if (req.priority === PRIORITY.CANCELLED) return
      }
      if (req.hash !== null && req.fork === fork && req.hash.nodes === 0) {
        req.hash.nodes = await MerkleTree.missingNodes(this.core.state.storage, req.hash.index, this.core.state.length)
        if (req.priority === PRIORITY.CANCELLED) return
