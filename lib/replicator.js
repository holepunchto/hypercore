...
class Peer {
  ...
  broadcastRange (start, length, drop) {
    ...
    // TODO: drop===true case
    // TODO: figure out the flow of setting _remoteContiguousLength
    if (!drop) {
      if (this._remoteContiguousLength >= start + length) return

      // TODO: more efficiently. Maybe a function directly in the bitfield logic?
      let nrFound = 0
      for (let i = start; i < start + length; i++) {
        if (!this.remoteBitfield.get(i)) break
        nrFound++
      }

      if (nrFound >= length) return
    }
