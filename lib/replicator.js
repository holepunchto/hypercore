...
class Peer {
  ...
  getMaxInflight () {
    const stream = this.stream.rawStream.stream || this.stream.rawStream
    if (!stream.udx) return this.maxInflight

    if (this.inflight === 0 || this.inflight === this.maxInflight) {
      // console.log('stream', { rtt: stream.rtt, inflight: stream.inflight }, 'inflight', { latency: this.latencyInflight, peer: this.inflight, max: this.maxInflight })
    }

    // this.maxInflight = (10 * Math.ceil((stream.inflight + 1) / (stream.rtt + 1))) / (latency + 1)
    // this.maxInflight = Math.ceil((100 * Math.ceil((stream.inflight + 1) / (stream.rtt + 1))))
    // this.maxInflight = Math.ceil((10 / (stream.rtt / stream.inflight)))
    // this.maxInflight = Math.ceil((stream.rtt || 1) / (stream.inflight || 1))
    // console.log('this.maxInflight', this.inflight + '/' + this.maxInflight)

    // this.maxInflight = Math.ceil((((stream.rtt + 1) * 100) / (this.latencyInflight + 1))) * DEFAULT_MAX_INFLIGHT

    // Progressive
    /* if (this.inflight === 0) {
      const max = (stream.rtt + 1) * DEFAULT_MAX_INFLIGHT
      if (max > this.maxInflight) this.maxInflight++
      else if (max < this.maxInflight) this.maxInflight--
    } */

    // this.maxInflight = (stream.rtt + 1) * DEFAULT_MAX_INFLIGHT

    const ratio = Math.max(0, Math.min(1, (stream.rtt - 50) / 500))
    this.maxInflight = DEFAULT_MAX_INFLIGHT + Math.floor(256 * ratio)
  ...
  async ondata (data) {
    ...
    if (req !== null) {
      ...
      const latency = Date.now() - req.$started
      this.latencyInflight = this.latencyInflight ? ((this.latencyInflight + latency) / 2) : latency
    }
